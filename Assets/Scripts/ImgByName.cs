...
{
    public static MyHashTable hashImagePath = new();
    ...
    public static MainImage getFromRms(string nameImg)
    {
        ...
        try
        {
            ...
            {
                nFrame = array[0],
                img = Image.createImage(array, 1, array.Length - 1)
            };
            if (result.img != null)
            {
            }
    ...
    {
        string text = mGraphics.zoomLevel + "ImgByName_" + nameImg;
        DataOutputStream dataOutputStream = new(data.Length + 1);
        int i = 0;
        try
        {
    ...
    {
        MyVector myVector = new("checkDelHash");
        if (isTrue)
        {
            hash.clear();
            return;
