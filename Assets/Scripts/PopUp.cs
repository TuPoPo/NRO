public class PopUp
{
    public static MyVector vPopups = new();
    ...
    public void paintClipPopUp(mGraphics g, int x, int y, int w, int h, int color, bool isFocus)
    {
        if (color == 1)
        {
            ...
            g.fillRect(x, y, w - 1, h, 10, 10, 10, 10);
        }
        else
        {
            g.setColor(255, 0.5f);
    ...
    public void paint(mGraphics g)
    {
        if (isPaint && says != null && ChatPopup.currChatPopup == null && !isHide)
        {
            ...
            {
                ((timeDelay != 0) ? mFont.tahoma_7b_blue : mFont.tahoma_7b_yellow).drawString(g, says[i], cx + (cw / 2), cy + ((ch / 2) - (says.Length * 12 / 2)) + (i * 12) - GameCanvas.transY, 2, mFont.tahoma_7b_dark);
            }
        }
    }
    ...
    private void update()
    {
        ...
        {
            isHide = cx + cw < GameScr.cmx || cx > GameCanvas.w + GameScr.cmx || cy + ch < GameScr.cmy || cy > GameCanvas.h + GameScr.cmy;
        }
        if (Char.myCharz().taskMaint == null || (Char.myCharz().taskMaint != null && Char.myCharz().taskMaint.taskId != 0))
        {
            isHide = cx + (cw / 2) < Char.myCharz().cx - 100 || cx + (cw / 2) > Char.myCharz().cx + 100 || cy + ch < GameScr.cmy || cy > GameCanvas.h + GameScr.cmy;
        }
        if (timeDelay > 0)
        {
            timeDelay--;
        ...
        if (Char.myCharz().taskMaint != null)
        {
            if (Char.myCharz().taskMaint.taskId == 0)
            {
                ...
                if (Char.myCharz().taskMaint.index is > 1 and < 6)
                {
                    isPaint = false;
                }
            }
