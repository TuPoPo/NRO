...
public class Hint
{
    ...
    {
        return Char.myCharz().taskMaint != null && Char.myCharz().taskMaint.taskId == tastId && Char.myCharz().taskMaint.index == index;
    }

    public static bool isPaintz()
    {
        return (!isOnTask(0, 3) || GameCanvas.panel.currentTabIndex != 0 || (GameCanvas.panel.cmy >= 0 && GameCanvas.panel.cmy <= 30)) && (!isOnTask(2, 0) || !GameCanvas.panel.isShow || GameCanvas.panel.currentTabIndex == 0);
    }
    ...
    public static void nextMap(int index)
    {
        if (!GameCanvas.panel.isShow && PopUp.vPopups.size() - 1 >= index)
        {
            ...
            y = popUp.cy + 30;
            isPaint = !popUp.isHide && popUp.isPaint;
            type = 0;
            isCamera = true;
            trans = 0;
