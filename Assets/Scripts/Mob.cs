...
public class Mob : IMapObject
{
    ...
    public int x;
    private static Mob myMob;
    public static Mob myMobz()
    {
        if (myMob == null)
        {
            myMob = new Mob();
    ...
    public virtual void update()
    {
        ...
        if ((isDie || hp <= 0 || status == 0 || status == 1) && mSystem.currentTimeMillis() - lastTimeDie > 1000)
        {
            timeDie--;
            lastTimeDie = mSystem.currentTimeMillis();
        }
        if (isMafuba)
        {
            return;
    ...
    {
        cTimeDie = mSystem.currentTimeMillis();
