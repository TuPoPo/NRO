...
public class mFont
{
    ...
    private readonly string strFont;

    private readonly int[][] fImages;
    ...
    {
        string text = "chelthm";
        if (id is (> 0 and < 10) or 19)
        {
            yAdd = 1;
            text = "barmeneb";
        }
        else if (id is >= 10 and <= 18)
        {
            text = "chelthm";
            yAdd = 2;
        }
    ...
    public void setTypePaint(mGraphics g, string st, int x, int y, int align, sbyte idFont, int size)
    {
        sbyte b = id;
        if (idFont > 0)
        {
        ...
        if (id > 24)
        {
            ...
            color1 = array[id - 25];
            color2 = array[id - 25];
            _drawString(g, st, x + 1, y, align, size);
            _drawString(g, st, x - 1, y, align, size);
            _drawString(g, st, x, y - 1, align, size);
            _drawString(g, st, x, y + 1, align, size);
            _drawString(g, st, x + 1, y + 1, align, size);
            _drawString(g, st, x + 1, y - 1, align, size);
            _drawString(g, st, x - 1, y - 1, align, size);
            _drawString(g, st, x - 1, y + 1, align, size);
            color1 = bigColor(id);
        ...
        _drawString(g, st, x, y - yAdd, align, size);
    }
    ...
    public void drawStringBorder(mGraphics g, string st, int x, int y, int align, int size)
    {
        if (mGraphics.zoomLevel == 1)
        {
            drawString(g, st, x, y, align);
        ...
        {
            setTypePaint(g, st, x, y, align, 0, size);
        }
    }
    ...
    public void drawStringBorder(mGraphics g, string st, int x, int y, int align, mFont font2, int size)
    {
        if (mGraphics.zoomLevel == 1)
        {
            drawString(g, st, x, y, align, font2);
        ...
        {
            drawStringBd(g, st, x, y, align, font2, size);
        }
    }
    public void drawStringBd(mGraphics g, string st, int x, int y, int align, mFont font, int size)
    {
        setTypePaint(g, st, x - 1, y - 1, align, font.id, size);
        setTypePaint(g, st, x - 1, y + 1, align, font.id, size);
        setTypePaint(g, st, x + 1, y - 1, align, font.id, size);
        setTypePaint(g, st, x + 1, y + 1, align, font.id, size);
        setTypePaint(g, st, x, y - 1, align, font.id, size);
        setTypePaint(g, st, x, y + 1, align, font.id, size);
        setTypePaint(g, st, x + 1, y, align, font.id, size);
        setTypePaint(g, st, x - 1, y, align, font.id, size);
        setTypePaint(g, st, x, y, align, 0, size);
    }
    ...
    public void _drawString(mGraphics g, string st, int x0, int y0, int align, int size)
    {
        y0 += yAddFont;
        GUIStyle gUIStyle = new(GUI.skin.label)
        {
    ...
    public static string[] splitStringSv(string _text, string _searchStr)
    {
        ...
        while (num3 != -1)
        {
            array[num5] = _text[num4..num3];
            num4 = num3 + length;
            num3 = _text.IndexOf(_searchStr, num4);
            num5++;
        }
        array[num5] = _text[num4..];
        return array;
    }
