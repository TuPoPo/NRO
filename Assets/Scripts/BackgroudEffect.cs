...
{
    public static MyVector vBgEffect = new();
    ...
    private readonly bool[] isRainEffect;

    private readonly int[] frame;

    private readonly int[] t;

    private readonly bool[] activeEff;
    ...
    private readonly int[] dem = new int[6] { 0, 1, 2, 1, 0, 0 };
    ...
    public BackgroudEffect(int typeS)
    {
        ...
        switch (typeEff)
        {
            ...
            case 4:
                {
                    ...
                    for (int m = 0; m < sum; m++)
                    {
                        ...
                        y[m] = Res.random(0, 50);
                        tick[m] = m % 2 == 0 ? 0 : m % 3 == 0 ? 1 : m % 4 == 0 ? 2 : 3;
                        t[m] = Res.random(0, 10);
                    }
                    break;
                }
            ...
            case 13:
                ...
                {
                    isPaintFar = Res.abs(Res.random(0, 2)) == 0;
    ...
    public static bool isHaveRain()
    {
        for (int i = 0; i < vBgEffect.size(); i++)
        {
            ...
            if (backgroudEffect.typeEff is 0 or 12)
            {
                return true;
            }
        }
            ...
            case 15:
                {
                    for (int j = 0; j < sum; j++)
                    {
                        ...
                        if (!activeEff[j])
                        {
                            ...
                            t[j]++;
                            _ = (typeEff != 11) ? 4 : 3;
                            int num = (typeEff != 15) ? 4 : 4;
                            if (t[j] > ((typeEff == 2) ? 4 : 2))
                            {
                                if (typeEff is not 11 and not 15)
                                {
                                    frame[j]++;
                                }
                                t[j] = 0;
    ...
    {
        BackgroudEffect backgroudEffect = new(10)
        {
            colorWater = color,
            yWater = yWater
        };
