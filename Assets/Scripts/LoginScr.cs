...
public class LoginScr : mScreen, IActionListener
{
    ...
    private readonly string numSupport = string.Empty;
    ...
    private readonly Command cmdCallHotline;
    ...
    public LoginScr()
    {
        ...
        GameScr.cmy = 200;
        Main.closeKeyBoard();
        defYL = GameCanvas.h > 200 ? GameCanvas.hh - 80 : GameCanvas.hh - 65;
        resetLogo();
        ...
        tfUser = new TField
        {
            y = GameCanvas.hh - mScreen.ITEM_HEIGHT - 9,
            width = wC,
    ...
    public void doLogin()
    {
        string text = Rms.loadRMSString("acc");
        string text2 = Rms.loadRMSString("pass");
        isLogin2 = (text == null || text.Equals(string.Empty)) && Rms.loadRMSString("userAo" + ServerListScreen.ipSelect) != null && !Rms.loadRMSString("userAo" + ServerListScreen.ipSelect).Equals(string.Empty);
    ...
    public override void keyPress(int keyCode)
    {
        if (tfUser.isFocus)
        {
            _ = tfUser.keyPressed(keyCode);
        }
        else if (tfPass.isFocus)
        {
            _ = tfPass.keyPressed(keyCode);
        }
    ...
    public override void paint(mGraphics g)
    {
        ...
        mFont.bigNumber_blue.drawString(g, "v" + GameMidlet.TuPoPo + "(" + mGraphics.zoomLevel + ")", GameCanvas.w - 2, 17, 1, mFont.tahoma_7_grey);
        if (mSystem.clientType == 1 && !GameCanvas.isTouch)
        {
            mFont.bigNumber_blue.drawString(g, ServerListScreen.linkweb, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
        }
        else
        {
            mFont.bigNumber_blue.drawString(g, ServerListScreen.linkweb, GameCanvas.w - 2, 2, 1, mFont.tahoma_7_grey);
        }
        ...
        if (GameCanvas.currentDialog == null)
        {
            ...
            if (GameCanvas.w < 176)
            {
                mFont.tahoma_8b.drawString(g, mResources.acc + ":", tfUser.x - 35, tfUser.y + 7, 0);
                mFont.tahoma_8b.drawString(g, mResources.pwd + ":", tfPass.x - 35, tfPass.y + 7, 0);
                mFont.tahoma_8b.drawString(g, mResources.server + ":" + serverName, GameCanvas.w / 2, tfPass.y + 32, 2);
            }
        }
        base.paint(g);
    }
    ...
    public override void updateKey()
    {
        ...
        if (!GameCanvas.isTouch)
        {
            right = tfUser.isFocus ? tfUser.cmdClear : tfPass.cmdClear;
        }
