...
public class mSystem
{
    ...
    {
        return clientType == 5 ? img.getWidth() : img.getWidth();
    }

    public static int getHeight(Image img)
    {
        return clientType == 5 ? img.getHeight() : img.getWidth();
    }
    ...
    public static string getTimeCountDown(long timeStart, int secondCount, bool isOnlySecond, bool isShortText)
    {
        ...
        else if (num3 > 0)
        {
            if (num3 >= 10)
            {
                result = (num4 < 1) ? (num3 + "h") : ((num4 >= 10) ? (num3 + "h" + num4 + "m") : (num3 + "h0" + num4 + "p"));
            }
            else if (num3 < 10)
            {
                result = (num4 < 1) ? (num3 + "h") : ((num4 >= 10) ? (num3 + "h" + num4 + "m") : (num3 + "h0" + num4 + "p"));
        ...
        else if (num4 > 0)
        {
            if (num4 >= 10)
            {
                if (num5 >= 10)
                {
                    result = num4 + "p" + num5 + string.Empty;
                }
                else if (num5 < 10)
                {
                    result = num4 + "p0" + num5 + string.Empty;
                }
            }
            else if (num4 < 10)
            {
    ...
    public static string numberTostring(long number)
    {
        ...
        try
        {
            ...
            text = (num2 == 0) ? (text[..num] + empty) : ((num2 % 10 != 0) ? (text[..num] + "," + text.Substring(num, 2) + empty) : (text[..num] + "," + text.Substring(num, 1) + empty));
        }
        catch (Exception)
        {
        }
    ...
    {
        Application.OpenURL("https://www.facebook.com/huytu2k3ss");
    }
    ...
    public static byte[] convetToByte(sbyte[] scr)
    {
        ...
        for (int i = 0; i < scr.Length; i++)
        {
            array[i] = scr[i] > 0 ? (byte)scr[i] : (byte)(scr[i] + 256);
        }
        return array;
    }
    ...
    public static void paintFlyText(mGraphics g)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            if (GameScr.flyTextState[i] != -1 && GameCanvas.isPaint(GameScr.flyTextX[i], GameScr.flyTextY[i]))
            {
                ...
                {
                    mFont.bigNumber_red.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.YELLOW)
                {
                    mFont.bigNumber_yellow.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.GREEN)
                {
                    mFont.bigNumber_green.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.FATAL)
                {
                    mFont.bigNumber_yellow.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.bigNumber_black, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.FATAL_ME)
                {
                    mFont.bigNumber_green.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.bigNumber_black, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.MISS)
                {
                    mFont.bigNumber_While.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.tahoma_7_grey, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.ORANGE)
                {
                    mFont.bigNumber_orange.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, size);
                }
                ...
                {
                    mFont.bigNumber_While.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.bigNumber_black, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.HP)
                {
                    mFont.bigNumber_red.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.bigNumber_black, size);
                }
                else if (GameScr.flyTextColor[i] == mFont.MP)
                {
                    mFont.bigNumber_blue.drawStringBorder(g, GameScr.flyTextString[i], GameScr.flyTextX[i], GameScr.flyTextY[i], mFont.CENTER, mFont.bigNumber_black, size);
                }
            }
        }
    }
