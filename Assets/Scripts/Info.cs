...
{
    public MyVector infoWaitToShow = new();
    ...
    public void paint(mGraphics g, int x, int y, int dir)
    {
        ...
        if (says != null && says.Length != 0 && type != 1)
        {
            ...
            for (int i = 0; i < says.Length; i++)
            {
                ...
                if (info.charInfo == null)
                {
                    mFont.tahoma_7.drawString(g, text, cx, cy - ch - 15 + sayRun + (i * 12) - (says.Length * 12) - 9, 2);
                    continue;
                }
                int num5 = X - 23;
                int num6 = Y - (num / 2);
                ...
                {
                    ((!info.isChatServer) ? mFont.tahoma_7b_greenSmall : mFont.tahoma_7b_yellowSmall2).drawString(g, (info.charInfo.isTichXanh ? "     " : string.Empty) + info.charInfo.cName + "[" + info.charInfo.charID + "]", X + 12, Y + 3, 0);
                    if (info.charInfo.isTichXanh)
                    {
                        TickXanh.PaintTicks(g, X + 8, Y + 2);
                    }
                }
                else
                {
