...
{
    private static readonly Image[] bgcam = new Image[8];

    private static readonly Image[] bgdo = new Image[8];

    private static readonly Image[] bgxanhla = new Image[8];

    private static readonly Image[] bgtim = new Image[8];

    private static readonly Image[] bgxanhnhat = new Image[8];

    private static readonly Image[] bgxanhdam = new Image[8];

    private static readonly Image[] effcam = new Image[8];

    private static readonly Image[] effdo = new Image[8];

    private static readonly Image[] effxanhla = new Image[8];

    private static readonly Image[] efftim = new Image[8];

    private static readonly Image[] effxanhnhat = new Image[8];

    private static readonly Image[] effxanhdam = new Image[8];
    ...
    public MyVector vPlayerMenu_id = new();
    public bool isShow; public int indexBoss;
    public int X;
    ...
    public MyVector vItemCombine = new();
    ...
    private readonly Command left = new(mResources.SELECT, 0);
    ...
    private static readonly string[][] boxZone = new string[1][] { mResources.zonee };

    private static readonly string[][] boxMap = new string[1][] { mResources.mapp };

    private static readonly string[][] boxGD = new string[3][]
    {
        mResources.inventory,
    ...
    public static sbyte BAG_BOX = 1;

    public static sbyte BOX_BODY = 2;

    public static sbyte BODY_BOX = 3;

    public static sbyte BAG_BODY = 4;

    public static sbyte BODY_BAG = 5;

    public static sbyte BAG_PET = 6;

    public static sbyte PET_BAG = 7;
    ...
    public MyVector logChat = new();

    public MyVector vPlayerMenu = new();

    public MyVector vFriend = new();

    public MyVector vMyGD = new();

    public MyVector vFriendGD = new();

    public MyVector vTop = new();

    public MyVector vEnemy = new();

    public MyVector vFlag = new();
    ...
    private readonly bool wantUpdateList;
    ...
    private readonly bool isHaveClan;
    ...
    private readonly int currentButtonPress;
    ...
    private readonly int[] zoneColor = new int[3] { 43520, 14743570, 14155776 };
    ...
    public static MyVector vGameInfo = new(string.Empty);
    ...
    public Panel()
    {
        ...
        {
            img = GameCanvas.loadImage("/mainImage/myTexture2dbtX.png"),
            cmdClosePanel = true
        };
    }
    ...
    public static void loadBg()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            if (bgcam[i] == null)
            {
                bgcam[i] = GameCanvas.loadImage("/inven/cam/bg/" + i);
            }
            if (bgdo[i] == null)
            {
                bgdo[i] = GameCanvas.loadImage("/inven/do/bg/" + i);
            }
            if (bgtim[i] == null)
            {
                bgtim[i] = GameCanvas.loadImage("/inven/tim/bg/" + i);
            }
            if (bgxanhdam[i] == null)
            {
                bgxanhdam[i] = GameCanvas.loadImage("/inven/xanhdam/bg/" + i);
            }
            if (bgxanhla[i] == null)
            {
                bgxanhla[i] = GameCanvas.loadImage("/inven/xanhla/bg/" + i);
            }
            if (bgxanhnhat[i] == null)
            {
                bgxanhnhat[i] = GameCanvas.loadImage("/inven/xanhnhat/bg/" + i);
            }
            if (effcam[i] == null)
            {
                effcam[i] = GameCanvas.loadImage("/inven/cam/eff/" + i);
            }
            if (effdo[i] == null)
            {
                effdo[i] = GameCanvas.loadImage("/inven/do/eff/" + i);
            }
            if (efftim[i] == null)
            {
                efftim[i] = GameCanvas.loadImage("/inven/tim/eff/" + i);
            }
            if (effxanhdam[i] == null)
            {
                effxanhdam[i] = GameCanvas.loadImage("/inven/xanhdam/eff/" + i);
            }
            if (effxanhla[i] == null)
            {
                effxanhla[i] = GameCanvas.loadImage("/inven/xanhla/eff/" + i);
            }
            if (effxanhnhat[i] == null)
            {
                effxanhnhat[i] = GameCanvas.loadImage("/inven/xanhnhat/eff/" + i);
            }
        }
    }
    ...
    {
        type = 2;
        boxTabName = GameCanvas.w > 2 * WIDTH_PANEL
            ? (new string[1][] { mResources.chestt })
            : (new string[2][]
            {
                mResources.chestt,
                mResources.inventory
    ...
    {
        type = 12;
        boxCombine = GameCanvas.w > 2 * WIDTH_PANEL
            ? (new string[1][] { mResources.combine })
            : (new string[2][]
            {
                mResources.combine,
                mResources.inventory
    ...
    {
        type = 21;
        boxPet = GameCanvas.panel2 != null ? mResources.petMainTab2 : mResources.petMainTab;
        tabName[21] = boxPet;
        strStatus = Char.myCharz().cgender == 1
            ? (new string[6]
            {
                mResources.follow,
                mResources.defend,
            ...
            if (item.itemOption != null)
            {
                for (int j = 0; j < item.itemOption.Length; j++)
                {
                    if (item.itemOption[j].optionTemplate.id == 41)
                    {
                        ...
                        {
                            text = text + "|0|1|" + "[" + item.template.id + "]" + item.template.name + text2;
                        }
                        if (item.itemOption[j].param == 2)
                        {
                            text = text + "|2|1|" + "[" + item.template.id + "]" + item.template.name + text2;
                        }
                        if (item.itemOption[j].param == 3)
                        {
                            text = text + "|8|1|" + "[" + item.template.id + "]" + item.template.name + text2;
                        }
                        if (item.itemOption[j].param == 4)
                        {
                            text = text + "|7|1|" + "[" + item.template.id + "]" + item.template.name + text2;
                        }
    ...
    private void paintGiaoDich(mGraphics g, bool isMe)
    {
        ...
        for (int i = 0; i < currentListLength; i++)
        {
            ...
            if (i == currentListLength - 3)
            {
                ...
                {
                    g.setColor((i != selected) ? 11393254 : 64154);
                }
                ...
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys((!isMe) ? friendMoneyGD : moneyGD) + " " + mResources.XU, num + 5, num2 + 11, 0);
            ...
            mFont.tahoma_7_yellow.drawStringBorder(g, i.ToString(), num - 35, num2, 0, mFont.tahoma_7b_dark);
            CustomGraphics.PaintItemOptions(g, this, item, num2);
            CustomGraphics.gI().paintInfoOption(g, item, num + 135, num2 + 10);
    ...
    {
        //SetDo.onUpdateScrollMousePanel(this, ref a);
        bool flag = false;
            ...
            clansOption = !isViewMember
                ? myMember.size() > 1
                    ...
                    mResources.memberr
                            ...
                            : (new string[1][] { mResources.memberr })
                : Char.myCharz().role > 0
                            ? (new string[2][]
                                    {
                mResources.msgg,
    ...
    {
        ITEM_HEIGHT = 24;
        currentListLength = currentTabIndex == currentTabName.Length - 1 && GameCanvas.panel2 == null && typeShop != 2
            ? checkCurrentListLength(Char.myCharz().arrItemBody.Length + Char.myCharz().arrItemBag.Length)
            : Char.myCharz().arrItemShop[currentTabIndex].Length;
        cmyLim = (currentListLength * ITEM_HEIGHT) - hScroll;
        if (cmyLim < 0)
        {
            cmyLim = 0;
    ...
    private void setTabSkillPet()
    {
        ITEM_HEIGHT = 30;
        int dem = 0;
        for (int i = 0; i < Char.myPetz().arrPetSkill.Length; i++)
        {
            if (Char.myPetz().arrPetSkill[i].template == null)
            {
                break;
            }
    ...
    private void setTabPetSkillInfo()
    {
        ITEM_HEIGHT = 30;
        currentListLength = Char.myPetz().arrPetSkill.Length + 5;
        cmyLim = (currentListLength * ITEM_HEIGHT) - hScroll;
        if (cmyLim < 0)
        {
            cmyLim = 0;
    ...
    public void moveCamera()
    {
        ...
        if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
        {
            ...
            if (isSelectPlayerMenu)
            {
                ...
                {
                    Char.myCharz().charFocus.menuSelect = selected - num < 0 ? selected : short.Parse((string)vPlayerMenu_id.elementAt(selected - num));
                }
    ...
    public void paintDetail(mGraphics g)
    {
        ...
        if (currItem != null && currItem.template.type != 5)
        {
            ...
            else if (currItem.compare is < 0 and not (-1))
            {
    ...
    public void paintTop(mGraphics g)
    {
        ...
        for (int i = num2; i < num; i++)
        {
            ...
            mFont.tahoma_7.drawString(g, mResources.rank + ": " + topInfo.rank + string.Empty, num6 + 5, num7 + 11, 0);
        }
        paintScrollArrow(g);
    }
    ...
    public void paint(mGraphics g)
    {
        ...
        switch (type)
        {
            ...
            case 23:
                ...
                break;
            case 32:
                PanelBoss.gI().paintListBosses(g);
                break;
            case 33:
                PanelBoss.gI().paintTabBoss(g);
                break;
            case 0:
                if (currentTabIndex == 0)
                {
            ...
            for (int i = 0; i < num; i++)
            {
                    ...
                    if (item.buyCoin != 0 || item.buyGold != 0)
                    {
                        if (typeShop != 2 && item.powerRequire == 0)
                        {
                            if (item.buyCoin > 0 && item.buyGold > 0)
                            {
                                if (item.buyCoin > 0)
                                {
                                    ...
                                    mFont.tahoma_7b_dark.drawString(g, Res.formatNumber(item.buyCoin), num2 + num4 - 15, num3 + 1, mFont.RIGHT);
                                }
                                if (item.buyGold > 0)
                                {
                        ...
                        if (typeShop == 2 && currentTabIndex <= 3 && !Equals(GameCanvas.panel2))
                        {
                            if (item.buyCoin > 0 && item.buyGold > 0)
                            {
                                ...
                                {
                                    g.drawImage(imgXu, num2 + num4 - 7, num3 + 7, 3);
                                    ((Char.myCharz().xu >= item.buyCoin) ? mFont.tahoma_7b_dark : mFont.tahoma_7b_red).drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 15, num3 + 1, mFont.RIGHT);
                                }
                                if (item.buyGold > 0)
                                {
                                    g.drawImage(imgLuong, num2 + num4 - 7, num3 + 7 + 11, 3);
                ...
                if (item.buyType == 1)
                {
                    ...
                    if (item.buyCoin != -1)
                    {
                        ...
                        mFont.tahoma_7b_dark.drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 15, num3 + 13, mFont.RIGHT);
                    }
                ...
                else if (item.buyType == 2)
                {
                    ...
                    if (item.buyCoin != -1)
                    {
                        ...
                        mFont.tahoma_7b_dark.drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 15, num3 + 11, mFont.RIGHT);
                    }
                    else if (item.buyGold != -1)
                    {
                        g.drawImage(imgLuongKhoa, num2 + num4 - 7, num3 + 17, 3);
    ...
    private void paintPetStatus(mGraphics g)
    {
        ...
        for (int i = 0; i < strStatus.Length; i++)
        {
            ...
            {
                g.setColor((i != selected) ? 11393254 : 64154);
                g.fillRect(x, num, num2, h, 10);
                mFont.tahoma_7b_red.drawString(g, strStatus[i], xScroll + 85, num + 6, mFont.CENTER);
            }
        }
        paintScrollArrow(g);
    }
    ...
    private void paintTools(mGraphics g)
    {
        ...
        for (int i = 0; i < strTool.Length; i++)
        {
            ...
            g.setColor((i != selected) ? 16711833 : 52428);
            g.fillRect(num, num2, num3, h, 20);

            mFont mFont = mFont.tahoma_7b_yellow;
            mFont.drawString(g, strTool[i], xScroll + (wScroll / 2), num2 + 6, mFont.CENTER);
            ...
            for (int j = 0; j < vGameInfo.size(); j++)
            {
                ...
                if (gameInfo.hasRead)
                {
                    ...
                    {
                        g.drawImage(imgNew, num + 10, num2 + 10 - 24, 3);
    ...
    private void paintSkill(mGraphics g)
    {
        ...
        for (int i = 0; i < num + 6; i++)
        {
            ...
            g.fillRect(num2, num3, num4, h, 10);
            ...
            if (i == 0)
            {
                ...
                mFont.tahoma_7b_red.drawString(g, st, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cHPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().hpFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 1)
            {
                ...
                mFont.tahoma_7b_red.drawString(g, st2, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cMPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().mpFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 2)
            {
                ...
                mFont.tahoma_7b_red.drawString(g, st3, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cDamGoc * 100) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().damFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 3)
            {
                ...
                mFont.tahoma_7b_red.drawString(g, st4, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(500000 + (Char.myCharz().cDefGoc * 100000)) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().defFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 4)
            {
                ...
                mFont.tahoma_7b_red.drawString(g, st5, num2 + 5, num3 + 3, 0);
                long number = num9;
                mFont.tahoma_7.drawString(g, Res.formatNumber2(number) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().criticalFrom1000Tiemnang, num2 + 5, num3 + 15, 0);
            }
            if (i == 5)
            {
                if (specialInfo != null)
                {
                    ...
                    {
                        mFont.tahoma_7.drawString(g, array[j], num2 + 5, num3 + 3 + (j * 12), 0);
                    }
                }
                else
                {
                    mFont.tahoma_7.drawString(g, string.Empty, num2 + 5, num3 + 9, 0);
                }
            }
            if (i < 6)
            {
            ...
            if (skill != null)
            {
                mFont.tahoma_7b_red.drawString(g, skillTemplate.name, num2 + 5, num3 + 3, 0);
                ...
                {
                    mFont.tahoma_7.drawString(g, mResources.max_level_reach, num2 + 5, num3 + 15, 0);
                }
                else if (skill.template.isSkillSpec())
                {
                    string text = mResources.proficiency + ": ";
                    int x = mFont.tahoma_7.getWidthExactOf(text) + num2 + 5;
                    int num12 = num3 + 15;
                    mFont.tahoma_7.drawString(g, text, num2 + 5, num12, 0);
                    mFont.tahoma_7.drawString(g, "(" + skill.strCurExp() + ")", num2 + num4 - 5, num12, mFont.RIGHT);
                    num12 += 4;
                    g.setColor(7169134);
                    g.fillRect(x, num12, 50, 5, 5);
                    int num13 = skill.curExp * 50 / 1000;
                    g.setColor(11992374);
                    g.fillRect(x, num12, num13, 5, 5);
                ...
                else
                {
                    ...
                    mFont.tahoma_7.drawString(g, mResources.level + " " + (skill.point + 1) + " " + mResources.need + " " + Res.formatNumber2(skill2.powRequire) + " " + mResources.potential, num2 + 5, num3 + 15, 0);
                }
            }
            else
            {
                ...
                mFont.tahoma_7.drawString(g, mResources.need_upper + " " + Res.formatNumber2(skill3.powRequire) + " " + mResources.potential_to_learn, num2 + 5, num3 + 15, 0);
            }
        }
        paintScrollArrow(g);
    }

    private void paintSkillPet(mGraphics g)
    {
        g.setColor(16711680);
        g.setClip(xScroll, yScroll, wScroll, hScroll);
        g.translate(0, -cmy);
        int num = Char.myPetz().arrPetSkill.Length;
        int skillnull = 0;
        for (int i = 0; i < num; i++)
        {
            SkillTemplate skillTemplate = Char.myPetz().arrPetSkill[i].template;
            if (skillTemplate == null)
            {
                skillnull++;
            }
        }
        Debug.Log("num: " + num + " skillnull: " + skillnull);
        num -= skillnull;
        //int num = Char.myPetz().nClass.skillTemplates.Length;
        for (int i = 0; i < num + 5; i++)
        {
            ...
            {
                SmallImage.drawSmallImage(g, 567, num5 + 4, num6 + 4, 0, 0);
                string st = mResources.HP + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myPetz().cHPGoc);
                mFont.tahoma_7b_red.drawString(g, st, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cHPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " 20", num2 + 5, num3 + 15, 0);
            }
            if (i == 1)
            {
                SmallImage.drawSmallImage(g, 569, num5 + 4, num6 + 4, 0, 0);
                string st2 = mResources.KI + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myPetz().cMPGoc);
                mFont.tahoma_7b_red.drawString(g, st2, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cMPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " 20", num2 + 5, num3 + 15, 0);
            }
            if (i == 2)
            {
                SmallImage.drawSmallImage(g, 568, num5 + 4, num6 + 4, 0, 0);
                string st3 = mResources.hit_point + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myPetz().cDamGoc);
                mFont.tahoma_7b_red.drawString(g, st3, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(Char.myPetz().cDamGoc * 100) + " " + mResources.potential + ": " + mResources.increase + " 1", num2 + 5, num3 + 15, 0);
            }
            if (i == 3)
            {
                SmallImage.drawSmallImage(g, 721, num5 + 4, num6 + 4, 0, 0);
                string st4 = mResources.armor + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myPetz().cDefGoc);
                mFont.tahoma_7b_red.drawString(g, st4, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7.drawString(g, NinjaUtil.getMoneys(500000 + (Char.myPetz().cDefGoc * 100000)) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().defFrom1000TiemNang, num2 + 5, num3 + 15, 0);
            }
            if (i == 4)
            {
                SmallImage.drawSmallImage(g, 719, num5 + 4, num6 + 4, 0, 0);
                ...
                long number = num9;
                mFont.tahoma_7.drawString(g, Res.formatNumber2(number) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myPetz().criticalFrom1000Tiemnang, num2 + 5, num3 + 15, 0);
            }
            ...
            int num11 = i - 5;
            SkillTemplate skillTemplate = Char.myPetz().arrPetSkill[num11].template;
            SmallImage.drawSmallImage(g, skillTemplate.iconId, num5 + 5, num6 + 4, 0, 0);
            Skill skill = Char.myPetz().arrPetSkill[num11];
            if (skill != null)
            {
                mFont.tahoma_7b_red.drawString(g, skillTemplate.name, num2 + 5, num3 + 3, 0);
                mFont.tahoma_7_blue.drawString(g, mResources.level + ": " + skill.point, num2 + num4 - 5, num3 + 3, mFont.RIGHT);
    ...
    private void paintMapTrans(mGraphics g)
    {
        ...
        for (int i = 0; i < mapNames.Length; i++)
        {
            ...
            _ = wScroll - 36;
            int h = ITEM_HEIGHT - 1;
            _ = yScroll + (i * ITEM_HEIGHT);
            //int num6 = 34;
            _ = ITEM_HEIGHT - 1;
            if (num2 - cmy <= yScroll + hScroll && num2 - cmy >= yScroll - ITEM_HEIGHT)
            {
                g.setColor((i != selected) ? 11393254 : 64154);
                g.fillRect(xScroll, num2, wScroll, h, 10);
                mFont.tahoma_7b_red.drawString(g, mapNames[i], 5, num2 + 1, 0);
                mFont.tahoma_7_blue.drawStringBd(g, planetNames[i], 5, num2 + 11, 0, mFont.tahoma_7_grey);
            }
        }
        paintScrollArrow(g);
    }
    ...
    private void paintZone(mGraphics g)
    {
        ...
        for (int i = 0; i < pts.Length; i++)
        {
            ...
            g.setColor((i != selected) ? 15196114 : 16383818);
            g.fillRect(num, num2, num3, h, 10);
            g.setColor(zoneColor[pts[i]]);
            g.fillRect(num4, y, num5, h2, 10);
            if (zones[i] != -1)
            {
                if (pts[i] != 1)
                {
                ...
                mFont.tahoma_7.drawString(g, GameScr.gI().numPlayer[i] + "/" + GameScr.gI().maxPlayer[i], num + 5, num2 + 6, 0);
            }
    ...
    private void paintSpeacialSkill(mGraphics g)
    {
        ...
        for (int i = num2; i < num; i++)
        {
            ...
            int h = ITEM_HEIGHT - 1;
            g.setColor((i != selected) ? 11393254 : 64154);
            g.fillRect(num7, num8, num9, h, 10);
            g.setColor((i != selected) ? 9993045 : 9541120);
            g.fillRect(num3, num4, num5, num6, 10);
            ...
            for (int i = 0; i < currentListLength; i++)
            {
                ...
                if (i == 0)
                {
                    for (int j = 0; j < num; j++)
                    {
                        ...
                        g.fillRect(xScroll + (j * TAB_W_NEW), num3 + 9 + num9, TAB_W_NEW - 1, 14, 5);
                ...
                CustomGraphics.gI().paintInfoOption(g, item, num2 + 135, num3 + 10);
                mFont2.drawString(g, "[" + item.template.id + "]" + item.template.name + text, num2 + 5, num3 + 1, 0);
                string text2 = string.Empty;
                if (item.itemOption != null)
                {
                    ...
                    if (item.itemOption.Length > 1)
                    {
                        ...
                        {
                            if ((item.itemOption[m] != null && !CustomGraphics.gI().getLogicOPT(item)) || item.template.type == 5 || (item.template.type > 5 && item.template.type != 32 && item.itemOption[m].optionTemplate.id != 102 && item.itemOption[m].optionTemplate.id != 107))
                            {
    ...
    public Member getCurrMember()
    {
        return selected < 2
            ? null
            : selected > ((member == null) ? myMember.size() : member.size()) + 1
            ? null
            : (member == null) ? ((Member)myMember.elementAt(selected - 2)) : ((Member)member.elementAt(selected - 2));
    }
    ...
    public ClanMessage getCurrMessage()
    {
        return selected < 2
            ? null
            : selected > ClanMessage.vMessage.size() + 1 ? null : (ClanMessage)ClanMessage.vMessage.elementAt(selected - 2);
    }

    public Clan getCurrClan()
    {
        return selected < 2 ? null : selected > clans.Length + 1 ? null : clans[selected - 2];
    }
    ...
    private void paintLogChat(mGraphics g)
    {
        ...
        if (logChat.size() == 0)
        {
            mFont.tahoma_7.drawString(g, mResources.no_msg, xScroll + (wScroll / 2), yScroll + (hScroll / 2) - (mFont.tahoma_7.getHeight() / 2) + 24, 2);
        }
        for (int i = 0; i < currentListLength; i++)
        {
            int num = xScroll;
            ...
            {
                g.setColor(15196114);
                g.fillRect(num, num5, wScroll, num7, 5);
            ...
            mFont.tahoma_7b_green2.drawString(g, (infoItem.charInfo.isTichXanh ? "     " : string.Empty) + infoItem.charInfo.cName + " - [" + infoItem.charInfo.charID + "]", num4 + 5, num5, 0);
            if (infoItem.charInfo.isTichXanh)
            {
                TickXanh.PaintTicks(g, num4 + 4, num5 + 1);
            }
    ...
    private void paintFlagChange(mGraphics g)
    {
        ...
        for (int i = 0; i < currentListLength; i++)
        {
            ...
            g.fillRect(num, num2, num3, h, 10);
            g.setColor((i != selected) ? 9993045 : 9541120);
            g.fillRect(num4, num5, num6, num7, 10);
            Item item = (Item)vFlag.elementAt(i);
            if (item == null)
            {
                continue;
            }
            mFont.tahoma_7.drawString(g, item.template.name, num + 5, num2 + 1, 0);
            string text = string.Empty;
            if (item.itemOption != null && item.itemOption.Length >= 1)
            {
                if (item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
    ...
    private void paintEnemy(mGraphics g)
    {
        ...
        if (currentListLength == 0)
        {
            mFont.tahoma_7.drawString(g, mResources.no_enemy, xScroll + (wScroll / 2), yScroll + (hScroll / 2) - (mFont.tahoma_7.getHeight() / 2), 2);
            return;
        }
        for (int i = 0; i < currentListLength; i++)
        {
            ...
            int h2 = ITEM_HEIGHT - 1;
            g.setColor((i != selected) ? 11393254 : 64154);
            g.fillRect(num4, num5, num6, h2, 10);
            g.setColor((i != selected) ? 9993045 : 9541120);
            g.fillRect(num, num2, num3, h, 10);
            ...
            {
                mFont.tahoma_7b_green.drawString(g, infoItem.charInfo.cName + " - [" + infoItem.charInfo.charID + "]", num4 + 5, num5, 0);
                mFont.tahoma_7_blue.drawString(g, infoItem.s, num4 + 5, num5 + 11, 0);
            }
            else
            {
                mFont.tahoma_7_grey.drawString(g, infoItem.charInfo.cName + " - [" + infoItem.charInfo.charID + "]", num4 + 5, num5, 0);
    ...
    private void paintFriend(mGraphics g)
    {
        ...
        if (currentListLength == 0)
        {
            mFont.tahoma_7.drawString(g, mResources.no_friend, xScroll + (wScroll / 2), yScroll + (hScroll / 2) - (mFont.tahoma_7.getHeight() / 2), 2);
            return;
        }
        for (int i = 0; i < currentListLength; i++)
        {
    ...
    private void paintClans(mGraphics g)
    {
        ...
        {
            mFont.tahoma_7.drawString(g, clanReport, xScroll + (wScroll / 2), yScroll + 24 + (hScroll / 2) - (mFont.tahoma_7.getHeight() / 2), 2);
        ...
        for (int j = 0; j < currentListLength; j++)
        {
            ...
            switch (j)
            {
                ...
                case 1:
                    g.setColor((j != selected) ? 11393254 : 64154);
                    g.fillRect(xScroll, num7, wScroll, num9, 10);
                    if (clanInfo != null)
                    {
            ...
            if (isSearchClan)
            {
                ...
                string st = (clans[j - 2].name.Length <= 23) ? clans[j - 2].name : (clans[j - 2].name[..23] + "...");
                ...
                g.setClip(xScroll, yScroll + cmy, wScroll, hScroll);
                mFont.tahoma_7.drawString(g, clans[j - 2].currMember + "/" + clans[j - 2].maxMember, num6 + num8 - 5, num7, mFont.RIGHT);
                continue;
            }
            if (isViewMember)
            {
            ...
            g.setColor((j != selected || clanMessage.option != null) ? 11393254 : 64154);
    ...
    private void paintArchivement(mGraphics g)
    {
        ...
        if (currentListLength == 0)
        {
            mFont.tahoma_7.drawString(g, mResources.no_mission, xScroll + (wScroll / 2), yScroll + (hScroll / 2) - (mFont.tahoma_7.getHeight() / 2), 2);
        }
        else
        {
        ...
        for (int j = 0; j < vItemCombine.size() + 1; j++)
        {
            ...
            if (j == vItemCombine.size())
            {
                if (vItemCombine.size() > 0)
                {
                    ...
                    {
                        g.setColor(16383818);
                        g.fillRect(num5, num2, wScroll, num4 + 2, 5);
                    }
    ...
    private void PaintLuoi(mGraphics g)
    {
        g.setColor(16711680);
        ...
        try
        {
            ...
            for (int j = 0; j < arrItemBody.Length; j++)
            {
                ...
                _ = GetInventorySelect_isbody(j, newSelected, Char.myCharz().arrItemBody);
                ...
                if (item.itemOption != null)
                {
                    for (int l = 0; l < item.itemOption.Length; l++)
                    {
                        if (item.itemOption[l].optionTemplate.id == 72)
                        {
                            if (item.itemOption[l].param is >= 1 and <= 5)
                            {
                                mFont2 = GetFont(0);
                            }
                            else if (item.itemOption[l].param is >= 6 and <= 7)
                            {
                                mFont2 = GetFont(8);
                            }
                            else if (item.itemOption[l].param is >= 8 and <= 10)
                            {
                                mFont2 = GetFont(7);
                            }
                        }
            ...
            for (int num11 = 0; num11 < arrItemBag.Length; num11++)
            {
                ...
                if (item2.itemOption != null)
                {
                    for (int num17 = 0; num17 < item2.itemOption.Length; num17++)
                    {
                        ...
                        {
                            if (item2.itemOption[num17].param is >= 1 and <= 5)
                            {
                                _ = GetFont(0);
                            }
                            else if (item2.itemOption[num17].param is >= 6 and <= 7)
                            {
                                _ = GetFont(8);
                            }
                            else if (item2.itemOption[num17].param is >= 8 and <= 10)
                            {
                                _ = GetFont(7);
                            }
                        }
                ...
                if (item2.quantity > 1)
                {
                    mFont.tahoma_7b_yellow.drawString(g, string.Empty + item2.quantity, x2 + num12, y2 + num15 - mFont.tahoma_7_yellow.getHeight(), 1);
                }
            }
        }
        catch (Exception)
    ...
    private void paintInventory(mGraphics g)
    {
        ...
        for (int i = 0; i < num18; i++)
        {
            ...
            g.setColor((i != newSelected) ? 16711833 : 16711935);
            g.fillRect(xScroll + (i * TAB_W_NEW), 89 + num20 - 10, TAB_W_NEW - 1, 21, 10);
            if (i == newSelected)
            {
                g.setColor(13524492);
                int x3 = xScroll + (i * TAB_W_NEW);
                int num21 = 89 + num20 - 10 + 21;
                g.fillRect(x3, num21 - 3, TAB_W_NEW - 1, 3, 10);
            }
            mFont.bigNumber_green.drawString(g, string.Empty + (i + 1), xScroll + (i * TAB_W_NEW) + (TAB_W_NEW / 2), 91 + num20 - 10, mFont.CENTER);
        }
        ...
        try
        {
            for (int j = 1; j < currentListLength; j++)
            {
                ...
                if (item3 != null && item3.template.type <= 5)
                {
                    bool num10 = AutoItem.set1.Contains(item3.x());
                    bool flag2 = AutoItem.set2.Contains(item3.x());
                    if (num10 && !flag2)
                    {
                ...
                paintEffectItem(g, item3, num26 - 2, num27 - 2);
                string text3 = string.Empty;
                mFont mFont4 = mFont.tahoma_7;
                if (item3.itemOption != null)
                {
                    for (int l = 0; l < item3.itemOption.Length; l++)
                    {
                        if (item3.itemOption[l].optionTemplate.id == 72)
                        {
                            ...
                            CustomGraphics.PaintItemEffectInPanel(g, num26 + 12, num27 + 11, num28, num29, item3);
                        }

                    }
                }
                if (AutoItem.set1.Contains(item3.x()))
                {
                    text3 = (!AutoItem.set2.Contains(item3.x())) ? (text3 + " - Set 1") : (text3 + " - Set 1,2");
                }
                else if (AutoItem.set2.Contains(item3.x()))
                {
                    text3 += " - Set 2";
                }
                mFont4.drawString(g, "[" + item3.template.id + "] " + item3.template.name + text3, num23 + 5, num24 + 1, 0);
                CustomGraphics.PaintItemOptions(g, this, item3, num24);
                CustomGraphics.gI().paintInfoOption(g, item3, num23 + 145, num24 + 13);
                ...
                if (item3.itemOption != null)
                {
                    ...
                    if (item3.itemOption.Length > 1)
                    {
                        for (int m = 1; m < 2; m++)
                        {
                            if ((item3.itemOption[m] != null && !CustomGraphics.gI().getLogicOPT(item3)) || item3.template.type == 5 || (item3.template.type > 5 && item3.template.type != 32 && item3.itemOption[m].optionTemplate.id != 102 && item3.itemOption[m].optionTemplate.id != 107))
                            {
                ...
                mFont.tahoma_7_yellow.drawStringBorder(g, j.ToString(), num23 - 23, num24, 0, mFont.tahoma_7b_dark);
    ...
    private void paintTab(mGraphics g)
    {
        if (type is 23 or 24)
        {
            ...
            {
                g.setColor((i != currentTabIndex) ? 11393254 : 64154);
                ...
                mFont mFont = (i != currentTabIndex) ? mFont.tahoma_7_grey : mFont.tahoma_7;
                if (!MenuHandler.MenuOption[i][1].Equals(string.Empty))
                {
                    mFont.drawString(g, MenuHandler.MenuOption[i][0], startTabPos + (i * TAB_W) + (TAB_W / 2), 53, mFont.CENTER);
                    mFont.drawString(g, MenuHandler.MenuOption[i][1], startTabPos + (i * TAB_W) + (TAB_W / 2), 64, mFont.CENTER);
                }
                else
                {
                    mFont.drawString(g, MenuHandler.MenuOption[i][0], startTabPos + (i * TAB_W) + (TAB_W / 2), 59, mFont.CENTER);
                }
                if (type == 0 && MenuHandler.MenuOption.Length == 5 && GameScr.isNewClanMessage && GameCanvas.gameTick % 4 == 0)
                {
        ...
        {
            mFont.tahoma_7b_red.drawString(g, "Thông báo BOSS", GameCanvas.panel.xScroll + (GameCanvas.panel.wScroll / 2), 59, mFont.CENTER, mFont.tahoma_7b_dark);
            g.setColor(13524492);
            g.fillRect(GameCanvas.panel.X + 1, 78, GameCanvas.panel.W - 2, 1, 5);
            return;
        }
        ...
        if (type is 27 or 28)
        {
            g.setColor(13524492);
            g.fillRect(X + 1, 78, W - 2, 1, 5);
            mFont.tahoma_7b_dark.drawString(g, "Thông Báo", xScroll + (wScroll / 2), 59, mFont.CENTER);
            return;
        }
        if (type == 18)
        {
        ...
        for (int i = 0; i < currentTabName.Length; i++)
        {
            ...
            mFont mFont2 = (i != currentTabIndex) ? mFont.tahoma_7_grey : mFont.tahoma_7;
    ...
    private void paintBottomMoneyInfo(mGraphics g)
    {
        if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
        {
            ...
            g.setColor(11837316);
            g.fillRect(X + 1, H - 15, W - 2, 14, 5);
            g.setColor(13524492);
            g.fillRect(X + 1, H - 15, W - 2, 1, 5);
    ...
    private void paintClanInfo(mGraphics g)
    {
        ...
        {
            Clan clan2 = currClan ?? Char.myCharz().clan;
    ...
    private void paintToolInfo(mGraphics g)
    {
        ...
        mFont.tahoma_7_yellow.drawString(g, (Char.myCharz().isTichXanh ? "     " : string.Empty) + Char.myCharz().cName + " - [" + Char.myCharz().charID + "]", 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
        if (Char.myCharz().isTichXanh)
        {
            TickXanh.PaintTicks(g, 58, 17);
        }
    ...
    {
        mFont.tahoma_7b_white.drawString(g, mResources.HP + ": " + NinjaUtil.getMoneys(Char.myPetz().cHP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cHPFull), X + 60, 4, mFont.LEFT, mFont.tahoma_8b);
        mFont.tahoma_7b_white.drawString(g, mResources.KI + ": " + NinjaUtil.getMoneys(Char.myPetz().cMP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cMPFull), X + 60, 16, mFont.LEFT, mFont.tahoma_8b);
        if (Char.myPetz().cDamFull > 0)
        {
            mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + ": " + NinjaUtil.getMoneys(Char.myPetz().cDamFull) + "     " + mResources.critical + ": " + NinjaUtil.getMoneys(Char.myPetz().cCriticalFull), X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
        }
        if (Char.myPetz().cMaxStamina > 0)
        {
            int cStamina = Char.myPetz().cStamina;
            int cMaxStamina = Char.myPetz().cMaxStamina;
            double percentage = (double)cStamina / cMaxStamina * 100;
            int roundedPercentage = (int)System.Math.Round(percentage);
            _ = Char.myPetz().cStamina / Char.myPetz().cMaxStamina * 100; ;
            mFont.tahoma_7_yellow.drawString(g, mResources.vitality + ": " + roundedPercentage + "%" + "   " + mResources.armor + ": " + NinjaUtil.getMoneys(Char.myPetz().cDefull), X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
        }
    }

    private void paintPetInfo(mGraphics g)
    {
        mFont.tahoma_7b_white.drawString(g, mResources.power + ": " + NinjaUtil.getMoneys(Char.myPetz().cPower), X + 60, 4, mFont.LEFT, mFont.tahoma_7_grey);
        mFont.tahoma_7b_white.drawString(g, mResources.potential + ": " + NinjaUtil.getMoneys(Char.myPetz().cTiemNang), X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
        if (Char.myPetz().cPower > 0)
        {
            mFont.tahoma_7_yellow.drawString(g, (!Char.myPetz().me) ? Char.myPetz().currStrLevel : Char.myPetz().getStrLevel(), X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
        }
    ...
    private void paintMapInfo(mGraphics g)
    {
        ...
        string text = string.Empty;
        if (TileMap.mapID is >= 135 and <= 138)
        {
    ...
    private void paintSkillPetInfo(mGraphics g)
    {
        ...
        mFont.tahoma_7_yellow.drawString(g, "Tiềm Năng" + ": " + NinjaUtil.getMoneys(Char.myPetz().cTiemNang), X + 60, 38, mFont.LEFT);
    }
    ...
    //    mFont.tahoma_7_yellow.drawString(g, mResources.armor + ": " + NinjaUtil.getMoneys(Char.myCharz().cDefull) + ", " + mResources.critical + ": " + NinjaUtil.getMoneys(Char.myCharz().cCriticalFull) + "%", X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
    //}
    private void paintItemBodyBagInfo(mGraphics g)
    {
        mFont.tahoma_7_yellow.drawString(g, mResources.HP + ": " + NinjaUtil.getMoneys(Char.myCharz().cHP) + " / " + NinjaUtil.getMoneys(Char.myCharz().cHPFull), X + 60, 2, mFont.LEFT, mFont.tahoma_7_grey);
        mFont.tahoma_7_yellow.drawString(g, mResources.KI + ": " + NinjaUtil.getMoneys(Char.myCharz().cMP) + " / " + NinjaUtil.getMoneys(Char.myCharz().cMPFull), X + 60, 14, mFont.LEFT, mFont.tahoma_7_grey);
        mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + ": " + NinjaUtil.getMoneys(Char.myCharz().cDamFull) + ", " + mResources.critical + ": " + NinjaUtil.getMoneys(Char.myCharz().cCriticalFull) + "%", X + 60, 26, mFont.LEFT, mFont.tahoma_7_grey);
        mFont.tahoma_7_yellow.drawString(g, mResources.giamsatthuong + ": " + NinjaUtil.getMoneys(Char.myCharz().cGiamST) + "%, " + mResources.critdame + ": " + NinjaUtil.getMoneys(Char.myCharz().cCritDameFull) + "%", X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
    }
    private void paintTopInfo(mGraphics g)
    {
        g.setClip(X + 1, Y, W - 2, yScroll - 2);
        g.setColor(9993045);
        g.fillRect(X, Y, W - 2, 50, 5);
        switch (type)
        {
            case 13:
                if (currentTabIndex is 0 or 1)
                {
                    if (Equals(GameCanvas.panel))
                    {
    ...
    public void paintTask(mGraphics g)
    {
        ...
        if (isPaintMap && !GameScr.gI().isMapDocNhan() && !GameScr.gI().isMapFize())
        {
            ...
            mFont.tahoma_8b.drawString(g, mResources.map, xScroll + (wScroll / 2), yScroll + hScroll - (num + 5), mFont.CENTER);
        }
        xstart = xScroll + 5;
        ystart = yScroll + 14;
        yPaint = ystart;
        ...
        if (indexMenu == 0)
        {
            ...
            if (Char.myCharz().taskMaint != null)
            {
                ...
                {
                    mFont.tahoma_7.drawString(g, Char.myCharz().taskMaint.details[k], xstart + 5, yPaint += 12, 0);
                    indexRowMax++;
                }
            }
            else
    ...
    public void hideNow()
    {
        ...
        {
            Command center = new(mResources.DIES[0], 11038, GameScr.gI());
    ...
    public void hide()
    {
        ...
        if (type != 4)
        {
            ...
            if (type == 33)
            {
                PanelBoss.gI().setTypeInfoBoss();
            }
            else if (type is 3 or 14 or 31 or 32)
            {
                if (isChangeZone)
                {
                    isClose = true;
            ...
            else if (type is 18 or 19 or 20 or 21)
            {
                setTypeMain();
                cmx = cmtoX = 0;
            }
            else if (type is 8 or 11 or 16)
            {
                setTypeAccount();
                cmx = cmtoX = 0;
            }
    ...
    private void doSpeacialSkill()
    {
        ...
        myVector.addElement(new Command("Mở max", AutoSpecialSkillFunctions.gI(), 4, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
    ...
    {
        g.setColor(0xFF0000); // Màu nền ban đầu
        g.setClip(xScroll, yScroll, wScroll, hScroll);
        g.translate(0, -cmy);

        Item[] arrItemBody = Char.myPetz().arrItemBody;
        Skill[] arrPetSkill = Char.myPetz().arrPetSkill;
        int total = arrItemBody.Length + arrPetSkill.Length;

        int spacing = 1;
        int adjustedHeight = ITEM_HEIGHT;
        int skillHeight = 30; // Chiều cao ô skill pet mới (to hơn adjustedHeight)

        int currentY = yScroll;

        for (int i = 0; i < total; i++)
        {
            bool isItem = i < arrItemBody.Length;
            int itemIndex = i;
            int skillIndex = i - arrItemBody.Length;

            int xName = xScroll + 24;
            int width = wScroll - 24;
            int xIcon = xScroll;
            int iconW = 24;

            int height = isItem ? adjustedHeight : skillHeight;
            int iconH = height;

            if (currentY - cmy > yScroll + hScroll || currentY - cmy < yScroll - (height + spacing))
            {
            ...
            if (isItem)
            {
                Item item = arrItemBody[itemIndex];
                // Vẽ nền
                g.setColor((i == selected) ? 16383818 : 15196114);
                g.fillRect(xName, currentY, width, height, 5);

                // Vẽ ô icon
                g.setColor((i == selected) ? 9541120 : 9993045);
                if (item != null)
                {
                    foreach (ItemOption opt in item.itemOption)
                    {
                        if (opt.optionTemplate.id == 72 && opt.param > 0)
                        {
                            sbyte color = GetColor_Item_Upgrade(opt.param);
                            int bgColor = GetColor_ItemBg(color);
                            if (bgColor != -1)
                            {
                                g.setColor(bgColor);
                            }
                ...
                g.fillRect(xIcon, currentY, iconW, iconH, 5);
                ...
                paintEffectItem(g, item, xIcon - 2, currentY - 2);

                if (item != null)
                {
                    string plus = "";
                    mFont fontName = mFont.tahoma_7;
                    ...
                    foreach (ItemOption opt in item.itemOption)
                    {
                        ...
                        if (opt.optionTemplate.id == 41)
                        {
                            fontName = GetFont(opt.param switch
                            {
                                1 => 0,
                                2 => 2,
                                3 => 8,
                    ...
                    CustomGraphics.PaintItemOptions(g, this, item, currentY);
                    CustomGraphics.gI().paintInfoOption(g, item, xName + 145, currentY + 11);
                    fontName.drawString(g, item.template.name + plus, xName + 5, currentY + 1, 0);

                    string options = "";
                    if (item.itemOption != null && item.itemOption.Length > 0)
                    {
                        if (item.itemOption[0].optionTemplate.id is not 102 and not 107)
                        {
                            options += item.itemOption[0].getOptionString();
                        }

                        for (int l = 1; l < System.Math.Min(2, item.itemOption.Length); l++)
                        {
                            if ((item.itemOption[l] != null && !CustomGraphics.gI().getLogicOPT(item)) || item.template.type == 5 || (item.template.type > 5 && item.template.type != 32 && item.itemOption[l].optionTemplate.id != 102 && item.itemOption[l].optionTemplate.id != 107))
                            {
                                options += "," + item.itemOption[l].getOptionString();
                            }
                        }
                        mFont fontOption = item.compare < 0 && item.template.type != 5 ? mFont.tahoma_7_red : mFont.tahoma_7_blue;
                        fontOption.drawString(g, options, xName + 5, currentY + 11, mFont.LEFT);
                    }
                    SmallImage.drawSmallImage(g, item.template.iconID, xIcon + (iconW / 2), currentY + (iconH / 2), 0, 3);

                    foreach (ItemOption opt in item.itemOption)
                    {
                        paintOptItem(g, opt.optionTemplate.id, opt.param, xIcon, currentY, iconW, iconH);
                        paintOptSlotItem(g, opt.optionTemplate.id, opt.param, xIcon, currentY, iconW, iconH);
                    }

                    if (item.quantity > 1)
                    {
                        mFont.tahoma_7_yellow.drawString(g, item.quantity.ToString(), xIcon + iconW, currentY + iconH - mFont.tahoma_7_yellow.getHeight(), 1);
                    }
                }
            }
            else
            {
                paintPetSkill(g, arrPetSkill[skillIndex], i, currentY, height - 1, iconW);
    ...
    private void setTabPetInventory()
    {
        ...
        int spacing = 1;
        int adjustedHeight = 24;
        int skillHeight = 30;

        int totalHeight = 0;
        for (int i = 0; i < arrItemBody.Length + arrPetSkill.Length; i++)
        {
            totalHeight += (i < arrItemBody.Length ? adjustedHeight : skillHeight) + spacing;
        }

        currentListLength = arrItemBody.Length + arrPetSkill.Length;
        cmyLim = totalHeight - hScroll;
        cmy = cmtoY = cmyLast[currentTabIndex];
        ...
        selected = (!GameCanvas.isTouch) ? 0 : -1;
    }

    private void paintPetSkill(mGraphics g, Skill skill, int index, int yRow, int height, int iconW)
    {
        int xName = xScroll + 28;
        int width = wScroll - 28;
        int xIcon = xScroll;

        // Vẽ nền
        g.setColor((index == selected) ? 16383818 : 15723751);
        g.fillRect(xName, yRow, width, height, 5);

        // Vẽ ô icon skill
        g.setColor((index == selected) ? 9541120 : 11837316);
        g.fillRect(xIcon, yRow, iconW, height - 1, 5);

        // Icon skill
        g.drawImage(GameScr.imgSkill, xIcon + (iconW / 2), yRow + (height / 2), 3);

        if (skill.template != null)
        {
            mFont.tahoma_7_blue.drawString(g, skill.template.name, xName + 5, yRow + 2, 0);
            mFont.tahoma_7.drawString(g, mResources.level + ": " + skill.point, xName + 5, yRow + 14, 0);
            SmallImage.drawSmallImage(g, skill.template.iconId, xIcon + (iconW / 2), yRow + (height / 2), 0, 3);
        }
        else
        {
            mFont.tahoma_7.drawString(g, skill.moreInfo, xName + 5, yRow + 5, 0);
            SmallImage.drawSmallImage(g, GameScr.efs[98].arrEfInfo[0].idImg, xIcon + (iconW / 2), yRow + (height / 2), 0, 3);
        }
    }
    ...
    private void doFireTop()
    {
        if (selected >= -1)
        {
            ...
            //    myVector.addElement(new Command(mResources.den, this, 8004, (InfoItem)vTop.elementAt(currInfoItem)));
            GameCanvas.menu.startAt(myVector, X, ((selected + 1) * ITEM_HEIGHT) - cmy + yScroll);
            addThachDauDetail((TopInfo)vTop.elementAt(selected));
        }
    }

    private void doFireBoss()
    {
        if (selected >= -1)
        {
            if (isThachDau)
            ...
            MyVector myVector = new(string.Empty);
            myVector.addElement(new Command("Dịch chuyển", this, 6901, (TopInfo)vTop.elementAt(selected)));
            myVector.addElement(new Command("Triệu hồi", this, 6902, (TopInfo)vTop.elementAt(selected)));
            myVector.addElement(new Command("Thông tin", this, 6903, (TopInfo)vTop.elementAt(selected)));
            GameCanvas.menu.startAt(myVector, X, ((selected + 1) * ITEM_HEIGHT) - cmy + yScroll);
            addThachDauDetail((TopInfo)vTop.elementAt(selected));
        }
    }
    ...
    private void doFireGiaoDich()
    {
        ...
        {
            currItem = Equals(GameCanvas.panel2)
                ? (Item)GameCanvas.panel2.vFriendGD.elementAt(selected)
                : (Item)GameCanvas.panel.vFriendGD.elementAt(selected);
    ...
    private void doFireShop()
    {
            ...
            if (currItem != null)
            {
                if (currItem.isBuySpec)
                {
                    if (currItem.buySpec > 0)
                    {
                        ...
                        //   myVector.addElement(new Command("Mua Nhiều", AutoItem.getInstance(), 3, new AutoItem.Item(currItem.template.id, -1, true, false)));
                    }
                }
                else if (typeShop == 4)
                {
        ...
        {
            currItem = type == 17 ? Char.myCharz().arrItemShop[4][selected] : Char.myCharz().arrItemShop[currentTabIndex][selected];
    ...
    private void doFireInventory()
    {
        ...
        else
        {
            ...
            if (!GetInventorySelect_isbody(selected, newSelected, Char.myCharz().arrItemBody))
            {
                ...
                if (item != null)
                {
                    bool num = AutoItem.set1.Contains(item.x());
                    bool flag = AutoItem.set2.Contains(item.x());
                    ...
                    else if (GameCanvas.panel.type == 13)
                    {
                        ...
                        myVector.addElement(new Command("SL: \nx" + currItem.quantity, this, 12345678, currItem));
                    }
                    else if (item.isTypeBody())
                    {
                        ...
                        if (Equals(GameCanvas.panel))
                        {
                            ...
                            if (!num && !flag)
                            {
                                myVector.addElement(new Command("Thêm Vào\nSet 1", AutoItem.getInstance(), 4, currItem));
                                myVector.addElement(new Command("Thêm Vào\nSet 2", AutoItem.getInstance(), 5, currItem));
                                if (num)
                                {
                                    goto IL_015e;
                                }
                        ...
                        IL_019a:
                            ...
                            {
                                myVector.addElement(new Command("Thêm Vào\nSet 1", AutoItem.getInstance(), 4, currItem));
                                myVector.addElement(new Command("Xóa Khỏi\nSet 2", AutoItem.getInstance(), 7, currItem));
                            }
                            if (num && flag)
                            {
                                myVector.addElement(new Command("Xóa Khỏi\nSet 1", AutoItem.getInstance(), 6, currItem));
                    ...
                    else
                    {
                        ...
                        if (AutoItem.isAutoUse(currItem.template.id))
                        {
                            myVector.addElement(new Command("Ngưng Auto\nSử Dụng", AutoItem.getInstance(), 2, (int)currItem.template.id));
                        }
                        else
                        {
                            myVector.addElement(new Command("Auto\nSử Dụng", AutoItem.getInstance(), 1, new AutoItem.ItemAuto(currItem.template.id, currItem.template.name)));
                        }
            ...
            {
                Char.myCharz().setPartTemp(currItem.headTemp, currItem.bodyTemp, currItem.legTemp, currItem.bagTemp);
                if (GameCanvas.panel.type is not 12 and not 13)
                {
                    if (position == 0)
                    {
    ...
    {
        currentListLength = MenuHandler.MenuOption.Length;
        ITEM_HEIGHT = 24;
        selected = GameCanvas.isTouch ? (-1) : 0;
        cmyLim = (currentListLength * ITEM_HEIGHT) - hScroll;
        if (cmyLim < 0)
    ...
    {
        string text = "|0|1|" + info.charInfo.cName + " - [" + info.charInfo.charID + "]";
        text += "\n";
    ...
    private void doFireSkill()
    {
        ...
        if (selected is 0 or 1 or 2 or 3 or 4 or 5)
        {
            ...
            if (selected == 0)
            {
                ...
                {
                    MyVector myVector2 = new(string.Empty);
            ...
            if (selected == 1)
            {
                ...
                {
                    MyVector myVector3 = new(string.Empty);
                ...
                {
                    MyVector myVector4 = new(string.Empty);
            ...
            if (selected == 2)
            {
                ...
                {
                    MyVector myVector5 = new(string.Empty);
                ...
                {
                    MyVector myVector6 = new(string.Empty);
            ...
            if (selected == 3)
            {
                ...
                MyVector myVector7 = new(string.Empty);
    ...
    private void doFireSkillPet()
    {
        ...
        if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4)
        {
            ...
            if (selected == 0)
            {
                ...
                if (cTiemNang > cHPGoc && cTiemNang < 10 * (2 * (cHPGoc + num2) + 180) / 2)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cHPGoc + num2) + mResources.use_potential_point_for2 + Char.myCharz().hpFrom1000TiemNang + mResources.for_HP, new Command(mResources.increase_upper, this, 9069, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * (cHPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    MyVector myVector = new MyVector(string.Empty);
                    myVector.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 9069, null));
                    myVector.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 9070, null));
                    GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2Pet(selected);
                }
                if (cTiemNang >= 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num2), this, 9069, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num2) + 180) / 2), this, 9070, null));
                    myVector2.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num2) + 1980) / 2), this, 9071, null));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2Pet(selected);
                }
            }
            if (selected == 1)
            {
                ...
                if (cTiemNang > cMPGoc && cTiemNang < 10 * (2 * (cMPGoc + num2) + 180) / 2)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cMPGoc + num2) + mResources.use_potential_point_for2 + Char.myCharz().mpFrom1000TiemNang + mResources.for_KI, new Command(mResources.increase_upper, this, 9069, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * (cMPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cMPGoc + num2) + 1980) / 2)
                {
                ...
                if (cTiemNang >= 100 * (2 * (cMPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cMPGoc + num2), this, 9069, null));
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cMPGoc + num2) + 180) / 2), this, 9070, null));
                    myVector4.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(100 * (2 * (cMPGoc + num2) + 1980) / 2), this, 9071, null));
                    GameCanvas.menu.startAt(myVector4, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2Pet(selected);
                }
            }
            if (selected == 2)
            {
                if (Char.myPetz().cTiemNang < Char.myPetz().cDamGoc * Char.myPetz().expForOneAdd)
                {
                ...
                if (cTiemNang > cDamGoc && cTiemNang < 10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd)
                {
                    GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + cDamGoc * 100 + mResources.use_potential_point_for2 + Char.myCharz().damFrom1000TiemNang + mResources.for_hit_point, new Command(mResources.increase_upper, this, 9069, null), new Command(mResources.CANCEL, this, 4007, null));
                    return;
                }
                if (cTiemNang >= 10 * (2 * cDamGoc + 9) / 2 * Char.myPetz().expForOneAdd && cTiemNang < 100 * (2 * cDamGoc + 99) / 2 * Char.myPetz().expForOneAdd)
                {
                    MyVector myVector5 = new MyVector(string.Empty);
                    myVector5.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 9069, null));
                    myVector5.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 9070, null));
                    GameCanvas.menu.startAt(myVector5, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2Pet(selected);
                }
                if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
                {
                    ...
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(cDamGoc * 100), this, 9069, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd), this, 9070, null));
                    myVector6.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + Res.formatNumber2(100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd), this, 9071, null));
                    GameCanvas.menu.startAt(myVector6, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2Pet(selected);
                }
            }
            if (selected == 3)
            {
                ...
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n1 " + mResources.armor + "\n" + Res.formatNumber2(number), this, 9069, null));
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n10 " + mResources.armor + "\n" + Res.formatNumber2(number2), this, 9070, null));
                myVector7.addElement(new Command(mResources.use_potential_point_for1 + "\n100 " + mResources.armor + "\n" + Res.formatNumber2(number3), this, 9071, null));
                GameCanvas.menu.startAt(myVector7, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                addSkillDetail2Pet(selected);
            }
            else if (selected == 4)
            {
                ...
                GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + Res.formatNumber(num3) + mResources.use_potential_point_for2 + Char.myPetz().criticalFrom1000Tiemnang + mResources.for_crit, new Command(mResources.increase_upper, this, 9069, null), new Command(mResources.CANCEL, this, 4007, null));
            }

            return;
        }
        int num5 = selected - 5;
        SkillTemplate skillTemplate = Char.myPetz().arrPetSkill[num5].template;
        Skill skill = Char.myPetz().arrPetSkill[num5];
        Skill skill2 = null;
        ...
        if (skill != null)
        {
            ...
            else
            {
                ...
                if (num5 == 0) nangCapSkill = 100;
                else if (num5 == 1) nangCapSkill = 150;
                else if (num5 == 2) nangCapSkill = 200;
                else if (num5 == 3) nangCapSkill = 500;
                myVector8.addElement(new Command(mResources.UPGRADE + "\n cần " + nangCapSkill + " ngọc", this, 9072, skill2));
            }
        }
        else
        {
    ...
    */
    ...
    private void addSkillDetail2Pet(int type)
    {
        string empty = string.Empty;
        long num = 0;
        if (selected == 0)
        {
            num = Char.myPetz().cHPGoc + 1000;
        }
        if (selected == 1)
        {
            num = Char.myPetz().cMPGoc + 1000;
        }
        if (selected == 2)
        {
            num = Char.myPetz().cDamGoc * Char.myPetz().expForOneAdd;
        }
        if (selected == 3)
        {
            num = 500000 + (Char.myPetz().cDefGoc * 100000);
        }
        string text = empty;
        empty = text + "|5|2|" + mResources.USE + " " + num + " " + mResources.potential;
        if (type == 0)
    ...
    {
        cp.says[^1] = mResources.received + " " + recieve + "/" + maxCap;
    }
    ...
    public void perform(int idAction, object p)
    {
        //if (idAction == 662003)
        //{
        //    Sellitem.ItemBan = currItem.template.id;
        //    Sellitem.NameItemBan = currItem.template.name;
        //    GameScr.info1.addInfo("Thêm Item" + currItem.template.name + ", id: " + currItem.template.id + " vị trí: ô số " + (currItem.indexUI + 1), 0);
        //}
        if (idAction == 12345678)
        {
            Item item2 = (Item)p;
            item2.isSelect = true;
            Item item3 = new()
            {
                ...
                indexUI = item2.indexUI,
                itemOption = item2.itemOption
            };
            GameCanvas.panel.vMyGD.addElement(item3);
            Service.gI().giaodich(2, -1, (sbyte)item3.indexUI, item3.quantity);
        }
        bool flag22 = idAction == 8010;
        if (flag22)
        {
            TFChat("Nhập Số Sao Cần Đập", "Chỉ Được Nhập Số");
        }
        if (idAction == 8011)
        {
            TFChat("Nhập Số Cấp Cần Đập", "Chỉ Được Nhập Số");
        }
        if (idAction == 9999)
        {
        ...
        {
            TopInfo topInfo = (TopInfo)p;
            Service.gI().sendTele(topInfo.pId);
        }
        if (idAction == 6902)
        {
            TopInfo topInfo = (TopInfo)p;
            Service.gI().sendSummon(topInfo.pId);
        }
        if (idAction == 6903)
        {
            TopInfo topInfo = (TopInfo)p;
            Service.gI().sendInfo(topInfo.name);
        }
        if (idAction == 3144)
        {
            AutoItem.AddItemstoList(currItem);
        }
        if (idAction == 170391)
        {
            Rms.clearAll();
        ...
        if (idAction == 6000)
        {
            ...
            //}
            //if (item2.template.type >= 0 && item2.template.type <= 5)
            //{
            //    AutoUpgradeFunctions.doDeDap = item2;
            //}
        }
        if (idAction == 7000)
        {
            ...
            Item item6 = new()
            {
                ...
                itemOption = item4.itemOption,
                indexUI = item4.indexUI
            };
        ...
        if (idAction == 8004)
        {
            TeleportMenu.TeleportToPlayer(((InfoItem)p).charInfo.charID);
        }
        if (idAction == 8005)
        {
            TeleportMenu.TeleportToPlayer((int)p);
        }
        if (idAction == 8001)
        {
        ...
        if (idAction == 2001)
        {
            ...
            sbyte b = inventorySelect_isbody ? ((sbyte)GetInventorySelect_body(selected, newSelected)) : ((sbyte)GetInventorySelect_bag(selected, newSelected, Char.myCharz().arrItemBody));
            Service.gI().useItem(0, (sbyte)((!inventorySelect_isbody) ? 1 : 0), b, -1);
            if (item9.template.id is 193 or 194)
            {
                GameCanvas.panel.hide();
            }
        }
        ...
        if (idAction == 2003)
        {
            ...
            sbyte b2 = inventorySelect_isbody2 ? ((sbyte)GetInventorySelect_body(selected, newSelected)) : ((sbyte)GetInventorySelect_bag(selected, newSelected, Char.myCharz().arrItemBody));
        ...
        if (idAction == 3002)
        {
            ...
            sbyte b3 = inventorySelect_isbody3 ? ((sbyte)GetInventorySelect_body(selected, newSelected)) : ((sbyte)GetInventorySelect_bag(selected, newSelected, Char.myCharz().arrItemBody));
        ...
        if (idAction == 9072)
        {
            Service.gI().upSkillPet(selected - 5, nangCapSkill);
            Service.gI().petInfo();
            GameCanvas.endDlg();
            InfoDlg.showWait();
        }
        ...
        if (idAction == 10012)
        {
            if (chatTField == null)
            {
                ...
                chatTField.tfChat.y = GameCanvas.h - 35 - ChatTextField.gI().tfChat.height;
                chatTField.initChatTextField();
                chatTField.parentScreen = GameCanvas.panel2 ?? GameCanvas.panel;
            }
    ...
    {
        Debug.Log("Tiêu đề: " + chatTField.strChat.ToString() + "Content: " + text + "TFChat: " + chatTField.tfChat.getText());
    ...
    private void updateCombineEff()
    {
            ...
            if (time > 0)
            {
                if (combineSuccess != -1)
                {
                    if (typeCombine == 3)
                    {
                        ...
                        {
                            Effect me = new(21, xS - 10, yS + 25, 4, 1, 1);
                            EffecMn.addEff(me);
                            time--;
                        }
                    }
        ...
        else
        {
            ...
            if (combineSuccess == 1)
            {
                if (countWait == 10)
                {
                    Effect me2 = new(22, xS - 3, yS + 25, 4, 1, 1);
                    EffecMn.addEff(me2);
                }
                countWait--;
                if (countWait < 0)
            ...
            else
            {
                ...
                if (countWait == 10)
                {
                    if (typeCombine == 2)
                    {
                        Effect me3 = new(20, xS - 3, yS + 15, 4, 2, 1);
                        EffecMn.addEff(me3);
                    }
                    else
                    {
                        Effect me4 = new(21, xS - 10, yS + 25, 4, 1, 1);
                        EffecMn.addEff(me4);
                    }
    ...
    private void setDotStar()
    {
        for (int i = 0; i < yArgS.Length; i++)
        {
            ...
            else if (angleS is >= 90 and < 180)
            {
                xDotS[i] = xS - xArgS[i];
                yDotS[i] = yS - yArgS[i];
            }
            else if (angleS is >= 180 and < 270)
            {
                xDotS[i] = xS - xArgS[i];
                yDotS[i] = yS + yArgS[i];
            }
    ...
    private void doNotiRuby(int type)
    {
        ...
        Command cmdYes = new(mResources.YES, this, (type != 0) ? 11001 : 11000, null);
        Command cmdNo = new(mResources.NO, this, 11002, null);
        ...
        try
        {
            ...
            for (int i = 0; i < nline; i++)
            {
                for (int j = 0; j < nsize; j++)
                {
                    ...
                    g.fillRect(x2, y2, sizeUpgradeEff[j], sizeUpgradeEff[j], 5);
                }
            }
        }
        catch (Exception)
    ...
    private static int upgradeEffectX(int dk, int tick, int wItem, int hitem, int wSize)
    {
        ...
        return 0 <= num && num < wItem
            ? num % wItem
            : wItem <= num && num < wItem + hitem
            ? wItem - wSize
            : wItem + hitem <= num && num < (wItem << 1) + hitem ? wItem - ((num - hitem) % wItem) - wSize : 0;
    }
    ...
    private static int upgradeEffectY(int dk, int tick, int wItem, int hitem, int wSize)
    {
        ...
        return 0 <= num && num < wItem
            ...
            : wItem <= num && num < wItem + hitem
            ? num % wItem
            : wItem + hitem <= num && num < (wItem << 1) + hitem ? hitem - wSize : hitem - ((num - (wItem << 1)) % hitem) - wSize;
    }
    ...
    public static sbyte GetColor_Item_Upgrade(int lv)
    {
        return lv < 0
            ...
            : (sbyte)(lv switch
            {
                0 or 1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 => 0,
                9 => 4,
                10 => 1,
                11 => 5,
                12 => 3,
    ...
    {
        return (type == 0 && currentTabIndex == 1) || (type == 7 && currentTabIndex == 0);
    }
