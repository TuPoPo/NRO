...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    private int lY;
    public static string tea = "Vũ trụ 1:112.213.94.23:14445:0,Vũ trụ 2:210.211.109.199:14445:0,Vũ trụ 3:112.213.85.88:14445:0,Vũ trụ 4:27.0.12.164:14445:0,Vũ trụ 5:27.0.12.16:14445:0,Vũ trụ 6:27.0.12.173:14445:0,Vũ trụ 7:112.213.94.223:14445:0,Võ đài liên vũ trụ:27.0.12.173:20000:0,Universe 1:54.179.255.27:14445:1,0,6";
    public static string smartPhoneVN = Server.textsv;
    public static string javaVN = Server.textsv;
    public static string smartPhoneIn = Server.textsv;
    public static string javaIn = Server.textsv;
    public static string smartPhoneE = Server.textsv;
    public static string javaE = Server.textsv;
    ...
    public override void paint(mGraphics g)
    {
        ...
        int num = 2;
        mFont.bigNumber_green.drawString(g, "v" + GameMidlet.TuPoPo + "(" + mGraphics.zoomLevel + ")", GameCanvas.w - 2, num + 15, 1, mFont.tahoma_7_grey);
        if (AutoLogin.waitToLogin)
        {
            mFont.tahoma_7b_yellow.drawStringBd(g, "Bắt đầu Login sau : " + AutoLogin.tWaitToLogin + " s", 50, 2, 0, mFont.tahoma_7b_dark, mFont.LEFT);
        }
        if (!isGetData || loadScreen)
        {
            if (mSystem.clientType == 1 && !GameCanvas.isTouch)
            {
                mFont.bigNumber_green.drawString(g, linkweb, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
            }
            else
            {
                mFont.bigNumber_green.drawString(g, linkweb, GameCanvas.w - 2, num, 1, mFont.tahoma_7_grey);
            }
        }
        else
        {
        ...
        if (cmdDeleteRMS != null)
        {
            mFont.bigNumber_red.drawString(g, mResources.xoadulieu, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
        }
            ...
            if (!loadScreen)
            {
                if (!bigOk)
                {
                    ...
                    else
                    {
                        ...
                        mFont.bigNumber_yellow.drawString(g, mResources.downloading_data + percent + "%", GameCanvas.w / 2, GameCanvas.hh + 24, 2);
    ...
    public void perform(int idAction, object p)
    {
        ...
        if (idAction == 3)
        {
            ...
            int num = ((Rms.loadRMSString("acc") != null) ? ((!Rms.loadRMSString("acc").Equals(string.Empty)) ? 1 : 0) : 0);
            bool flag = Rms.loadRMSString("userAo" + ipSelect) != null && ((!Rms.loadRMSString("userAo" + ipSelect).Equals(string.Empty)) ? true : false);
            if (num == 0 && !flag)
            {
                GameCanvas.connect();
