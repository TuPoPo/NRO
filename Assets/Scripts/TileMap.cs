...
public class TileMap
{
    ...
    public static MyVector vGo = new();

    public static MyVector vItemBg = new();

    public static MyVector vCurrItem = new();
    ...
    public static MyVector vObject = new();
    ...
    private static readonly int[] colorMini = new int[2] { 5257738, 8807192 };
    ...
    {
        return mapID is 51 or 103 or 112 or 113 or 129 or 130;
    }

    public static bool isTrainingMap()
    {
        return mapID is 39 or 40 or 41;
    }

    public static bool mapPhuBang()
    {
        return GameScr.phuban_Info != null && mapID == GameScr.phuban_Info.idmapPaint;
    }
    ...
    {
        if (id is 156 or 330 or 345 or 334)
        {
            return false;
        }
        if (mapID is 54 or 55 or 56 or 57 or 58 or 59 or 103)
        {
            return false;
        }
        int num = 0;
    ...
    {
        return mapID is 45 or 46 or 48;
    }

    public static bool isDoubleMap()
    {
        return isMapDouble || mapID == 45 || mapID == 46 || mapID == 48 || mapID == 51 || mapID == 52 || mapID == 103 || mapID == 112 || mapID == 113 || mapID == 115 || mapID == 117 || mapID == 118 || mapID == 119 || mapID == 120 || mapID == 121 || mapID == 125 || mapID == 129 || mapID == 130;
    }
    ...
    public static void getTile()
    {
        ...
        if (mGraphics.zoomLevel == 1)
        {
            ...
            {
                string empty = (k >= 9) ? ("/t/" + tileID + "/t_" + (k + 1)) : ("/t/" + tileID + "/t_0" + (k + 1));
            ...
            for (int n = GameScr.gssx; n < GameScr.gssxe; n++)
            {
                for (int num5 = GameScr.gssy; num5 < GameScr.gssye; num5++)
                {
                    ...
                    if (flag18)
                    {
                        ...
                        if (flag19)
                        {
                            g.drawRect(n * size, (num5 * size) + 8, 24, 24);
                        }
                        else
                        {
                            g.fillRect(n * size, (num5 * size) + 8, 24, 24);
                        }
                    }
                }
            }
    ...
    {
        return mapID is not 54 and not 55 and not 56 and not 57 and not 138 and not 167;
    }
