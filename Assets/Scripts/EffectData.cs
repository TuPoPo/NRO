...
public class EffectData
{
    ...
    public short[] get(int index)
    {
        ...
        return anim_data[index] ?? (new short[1]);
    }
    ...
    public void readEffect(myReader msg)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < b; i++)
            {
                ...
                {
                    ID = msg.readByte(),
                    x0 = msg.readUnsignedByte(),
                    y0 = msg.readUnsignedByte(),
                    w = msg.readUnsignedByte(),
                    h = msg.readUnsignedByte()
                };
            }
    ...
    public void readData(myReader iss)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < b; i++)
            {
                ...
                {
                    ID = iss.readByte(),
                    x0 = iss.readUnsignedByte(),
                    y0 = iss.readUnsignedByte(),
                    w = iss.readUnsignedByte(),
                    h = iss.readUnsignedByte()
                };
            }
    ...
    public void paintFrame(mGraphics g, int f, int x, int y, int trans, int layer)
    {
        ...
        for (int i = 0; i < frame.dx.Length; i++)
        {
            ...
            try
            {
                ...
                if (trans == 0)
                {
                    g.drawRegion(img, imageInfo.x0, imageInfo.y0, imageInfo.w, imageInfo.h, 0, x + frame.dx[i], y + frame.dy[i] - ((layer is < 4 and > 0) ? GameCanvas.transY : 0), 0);
                }
                if (trans == 1)
                {
                    g.drawRegion(img, imageInfo.x0, imageInfo.y0, imageInfo.w, imageInfo.h, 2, x - frame.dx[i], y + frame.dy[i] - ((layer is < 4 and > 0) ? GameCanvas.transY : 0), StaticObj.TOP_RIGHT);
                }
                if (trans == 2)
                {
                    g.drawRegion(img, imageInfo.x0, imageInfo.y0, imageInfo.w, imageInfo.h, 7, x - frame.dx[i], y + frame.dy[i] - ((layer is < 4 and > 0) ? GameCanvas.transY : 0), StaticObj.VCENTER_HCENTER);
                }
            }
            catch (Exception)
            {
