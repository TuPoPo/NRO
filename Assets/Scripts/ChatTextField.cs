...
public class ChatTextField : IActionListener
{
    ...
    public string strChat = "Mod By TuPoPo: ";
    ...
    public void keyPressed(int keyCode)
    {
        if (isShow)
        {
            _ = tfChat.keyPressed(keyCode);
        }
        right.caption = tfChat.getText().Equals(string.Empty) ? mResources.CLOSE : mResources.DELETE;
    }
    ...
    public void startChat(int firstCharacter, IChatable parentScreen, string to)
    {
        ...
        _ = tfChat.keyPressed(firstCharacter);
    ...
    public void paint(mGraphics g)
    {
        if (isShow && !Main.isIPhone)
        {
            ...
            if (Main.isPC)
            {
                mFont.tahoma_7b_red.drawString(g, strChat + to, tfChat.x, tfChat.y - ((!GameCanvas.isTouch) ? 12 : 17), 0);
