  ...
  //  private static bool isShowErrorForm = false;
    ...
    public void update()
    {
        ...
        try
        {
            ...
            if (GameCanvas.gameTick > 10000)
            {
                ...
                {
                    if (LoginTuPoPo.isAutoLogin)
                    {
                        new Thread(new ThreadStart(LoginTuPoPo.AutoLogin)).Start();
                    }
                    else
                    {
    ...
    public void onDisconnected()
    {
        ...
        if (flag)
        {
            ...
            if (flag2)
            {
                bool isAutoLogin = LoginTuPoPo.isAutoLogin;
                if (isAutoLogin)
                {
                    new Thread(new ThreadStart(LoginTuPoPo.AutoLogin)).Start();
                }
        ...
        {
            bool isAutoLogin2 = LoginTuPoPo.isAutoLogin;
            if (isAutoLogin2)
            {
                new Thread(new ThreadStart(LoginTuPoPo.AutoLogin)).Start();
            }
    ...
    {
        PaintGameScr.paintgamecanvas(g);
    }
    ...
    public void paintChangeMap(mGraphics g)
    {
        ...
        g.setColor(11584734);
        g.fillRect(0, 0, w, h);
        g.drawImage(LoginScr.imgTitle, w / 2, h / 2 - 5, StaticObj.BOTTOM_HCENTER);
        paintShukiren(hw, h / 2 + 24, g);
        mFont.bigNumber_green.drawString(g, mResources.PLEASEWAIT + ((LoginScr.timeLogin <= 0) ? string.Empty : (" " + LoginScr.timeLogin + "s")), w / 2, h / 2, 2);
    }
