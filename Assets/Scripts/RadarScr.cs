...
public class RadarScr : mScreen
{
    ...
    private static readonly int[] index = new int[5] { -2, -1, 0, 1, 2 };

    private int dyArrow;

    private readonly int[] dxArrow;
    ...
    private readonly int xClip;

    private int wClip;

    private readonly int yClip;
    ...
    public void SetRadarScr(MyVector list, int num, int numMax)
    {
        ...
        TYPE_UI = true;
        SetListUse();
        maxpage = TYPE_UI ? (list.size() / 5) + ((list.size() % 5 > 0) ? 1 : 0) : (listUse.size() / 5) + ((listUse.size() % 5 > 0) ? 1 : 0);
    }
