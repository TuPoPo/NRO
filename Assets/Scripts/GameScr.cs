...
public class GameScr : mScreen, IChatable
{
    ...
    public static MyVector textTime = new(string.Empty);
    ...
    public static Image[] imgLogoGif;
    ...
    private readonly int idTypeTask;

    private bool isstarOpen;

    private readonly bool isChangeSkill;

    public static MyVector vClan = new();

    public static MyVector vPtMap = new();

    public static MyVector vFriend = new();

    public static MyVector vEnemies = new();

    public static MyVector vCharInMap = new();

    public static MyVector vItemMap = new();

    public static MyVector vMobAttack = new();

    public static MyVector vSet = new();

    public static MyVector vMob = new();

    public static MyVector vNpc = new();

    public static MyVector vFlag = new();
    ...
    private static readonly Scroll scrInfo = new();

    public static Scroll scrMain = new();

    public static MyVector vItemUpGrade = new();
    ...
    public static InfoMe info1 = new();

    public static InfoMe info2 = new();
    ...
    public static GamePad gamePad = new();

    public static Image imgChatPC;

    public static Image imgChatsPC2;

    public static Image imgTDLT;

    public static Image imgPorata;

    public static Image imgCapsule;

    public static Image imgSetting;

    public static Image imgTudanh;

    public static Image imgDoiKhu;

    public static Image imgXmapLeft;

    public static Image imgXmapRight;

    public static Image imgXmapUp;

    public static Image imgAutoHS;

    public static Image imgSlVang;

    public static Image imgSlNgocXanh;

    public static Image imgSlHongNgoc;
    ...
    private readonly long lastMove;
    ...
    private readonly string strTam = string.Empty;
    ...
    private readonly long longPress;
    ...
    private readonly long lastSendUpdatePostion;

    public static long lastTick;

    public static long currTick;

    private readonly int timeAuto;
    ...
    private static readonly int[] shock_x = new int[4] { 1, -1, 1, -1 };

    private static readonly int[] shock_y = new int[4] { 1, -1, -1, 1 };
    ...
    private static readonly int xL;

    private static readonly int yL;

    public int xR;

    public int yR;

    private static readonly int xU;

    private static readonly int yU;
    ...
    private readonly int[] idVS = new int[2] { -1, -1 };
    ...
    private static readonly Image imgNolearn;
    ...
    private readonly int indexTiemNang;
    ...
    private readonly int totalRowInfo;

    private readonly int ypaintKill;

    private readonly int ylimUp;

    private readonly int ylimDow;

    private readonly int yPaint;
    ...
    private readonly int isTranKyNang;

    private readonly bool isTran;

    private readonly int cmY_Old;

    private readonly int cmX_Old;

    public PopUpYesNo popUpYesNo;

    public static MyVector vChatVip = new();
    ...
    public static bool isPaintPhimTatEnabled = false;
    ...
    public static void loadBg()
    {
        ...
        imgChatPC = GameCanvas.loadImage("/mainImage/chat.png");
        imgChatsPC2 = GameCanvas.loadImage("/mainImage/chat2.png");
        imgSetting = GameCanvas.loadImage("/mainImage/balo.png");
        imgSlNgocXanh = GameCanvas.loadImage("/mainImage/myTexture2dimgDiamond.png");
        imgSlHongNgoc = GameCanvas.loadImage("/mainImage/luongkhoa.png");
        ...
        MobCapcha.init();
        isAnalog = ActionPerform.IsMobile() ? 1 : 0;
        //isAnalog = 1;
        gamePad = new GamePad();
    ...
    {
        return TileMap.mapID is >= 53 and <= 62;
    }
    ...
    public void saveonScreenSkillToRMS()
    {
        ...
        {
            array[i] = onScreenSkill[i] == null ? (sbyte)-1 : onScreenSkill[i].template.id;
        }
    ...
    public void saveKeySkillToRMS()
    {
        ...
        {
            array[i] = keySkill[i] == null ? (sbyte)-1 : keySkill[i].template.id;
        }
        Service.gI().changeOnKeyScr(array);
    }
    ...
    public void readEfect()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                {
                    idEf = dataInputStream.readShort(),
                    arrEfInfo = new EffectInfoPaint[dataInputStream.readByte()]
                };
                for (int j = 0; j < efs[i].arrEfInfo.Length; j++)
                {
                    efs[i].arrEfInfo[j] = new EffectInfoPaint
                    {
                        idImg = dataInputStream.readShort(),
                        dx = dataInputStream.readByte(),
                        dy = dataInputStream.readByte()
                    };
                }
    ...
    public void playerMenu(Char c)
    {
        ...
        if (Char.myCharz().charFocus.statusMe is not 14 and not 5)
        {
    ...
    public bool isAttack()
    {
        ...
        if (Char.myCharz().mobFocus != null)
        {
            ...
            if (num <= Char.myCharz().myskill.dx && num2 <= Char.myCharz().myskill.dy)
            {
                ...
                int num3 = 20;
                bool flag = false;
                if (Char.myCharz().mobFocus is BigBoss or BigBoss2)
                {
                    flag = true;
                }
        ...
        if (Char.myCharz().npcFocus != null)
        {
            ...
            Char.myCharz().npcFocus.cdir = Char.myCharz().cx < Char.myCharz().npcFocus.cx ? -1 : 1;
    ...
    {
        return (Char.myCharz().myskill == null || ((Char.myCharz().myskill.template.manaUseType == 1 || Char.myCharz().cMP >= Char.myCharz().myskill.manaUse) && (Char.myCharz().myskill.template.manaUseType != 1 || Char.myCharz().cMP >= Char.myCharz().cMPFull * Char.myCharz().myskill.manaUse / 100))) && Char.myCharz().myskill != null && (Char.myCharz().myskill.template.maxPoint <= 0 || Char.myCharz().myskill.point != 0);
    }
    ...
    public override void updateKey()
    {
        ...
        if (GameCanvas.isTouch && !ChatTextField.gI().isShow && !GameCanvas.menu.showMenu)
        {
            ...
            UpdateGameScr.updatekeytouchcontrol();
        }
        checkAuto();
    ...
    private bool checkClipTopChatPopUp(int xClick, int yClick)
    {
        ...
        {
            int num = Res.abs(info2.cmx) + info2.info.X - 40;
            int num2 = Res.abs(info2.cmy) + info2.info.Y;
    ...
    public void activeRongThanEff(bool isMe)
    {
        ...
        if (isMe)
        {
            Effect me = new(20, Char.myCharz().cx, Char.myCharz().cy - 77, 2, 8, 1);
            EffecMn.addEff(me);
        }
    }
    ...
    public void doiMauTroi()
    {
        ...
        mautroi = mGraphics.blendColor(0.4f, 0, GameCanvas.colorTop[^1]);
    }
    ...
    public void callRongThan(int x, int y)
    {
        ...
        doiMauTroi();
        Effect me = new((!isRongNamek) ? 17 : 25, x, y - 77, 2, -1, 1);
        EffecMn.addEff(me);
    }
    ...
    private void autoPlay()
    {
        ...
        for (int i = 0; i < vMob.size(); i++)
        {
            ...
            if (mob.status is not 0 and not 1)
            {
                flag = true;
            }
        }
    ...
    {
        Npc npc = new(5, 0, -100, 100, 5, info1.charId[Char.myCharz().cgender][2]);
        ...
        _ = ChatPopup.addChatPopupWithIcon(nhatvatpham, 100000, npc, 5820);
    }
    ...
    {
        return Char.myCharz().isFlyAndCharge || Char.myCharz().isUseSkillAfterCharge || Char.myCharz().isStandAndCharge || Char.myCharz().isWaitMonkey || isSuperPower;
    }
    ...
    public void checkMouseChat()
    {
        ...
        {
            mScreen.keyMouse = cmdMenu != null && GameCanvas.isMouseFocus(cmdMenu.x, cmdMenu.y, cmdMenu.w / 2, cmdMenu.h) ? 1 : -1;
        }
    }
    ...
    public override void paint(mGraphics g)
    {
        ...
        if (TileMap.mapID == 120)
        {
            ...
            if (mabuEff)
            {
                ...
                {
                    Effect me = new(19, Res.random((TileMap.pxw / 2) - 50, (TileMap.pxw / 2) + 50), 340, 2, 1, -1);
                    EffecMn.addEff(me);
                }
                if (GameCanvas.gameTick % 15 == 0)
                {
                    Effect me2 = new(18, Res.random((TileMap.pxw / 2) - 5, (TileMap.pxw / 2) + 5), Res.random(300, 320), 2, 1, -1);
                    EffecMn.addEff(me2);
                }
                if (tMabuEff == 100)
                {
        ...
        if (!isPaintOther)
        {
            ...
            for (int i = 0; i < textTime.size(); i++)
            {
                bool hide = ((ItemTime)textTime.elementAt(i)).text.ToLower().Contains("bà hạt mít");
                if (!hide)
                {
    ...
    private void checkEffToObj(IMapObject obj)
    {
        if (obj != null && tDoubleDelay <= 0)
        {
            ...
            int num = (num2 <= 80) ? 1 : ((num2 is > 80 and <= 200) ? 2 : ((num2 is <= 200 or > 400) ? 4 : 3));
            Res.outz("nLoop= " + num);
    ...
    private void paintWaypointArrow(mGraphics g)
    {
        ...
        for (int i = 0; i < TileMap.vGo.size(); i++)
        {
            ...
            else if (waypoint.minX is >= 0 and < 24)
            {
                if (!GameCanvas.isTouch)
                {
    ...
    public bool isVS()
    {
        return TileMap.isVoDaiMap() && (Char.myCharz().cTypePk != 0 || (TileMap.mapID == 130 && findCharVS1() != null && findCharVS2() != null));
    }
    private void paintSelectedSkill(mGraphics g)
    {

        bool flag = mobCapcha != null;
        if (flag)
        {
            paintCapcha(g);
        }
        ...
        {
            bool flag2 = GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu || isPaintPopup() || GameCanvas.panel.isShow || Char.myCharz().taskMaint.taskId == 0 || ChatTextField.gI().isShow || GameCanvas.currentScreen == MoneyCharge.instance;
            if (!flag2)
            {
                long num = mSystem.currentTimeMillis();
                ...
                {
                    bool flag6 = Char.myCharz().statusMe != 14;
                    if (flag6)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                g.setColor(16777215);
                                g.fillRect(GameScr.xHP + 10, GameScr.yHP + 10 + ((num3 != 0) ? (20 - num3) : 0) - 6, 20, (num3 == 0) ? 18 : num3);
                ...
                if (flag11)
                {
                    ...
                    for (int i = 0; i < num4; i++)
                    {
                        ...
                        if (isPC)
                        {
                            ...
                            {
                                mFont.bigNumber_blue.drawString(g, array5[i], GameScr.xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + num5, mFont.CENTER);
                                mFont.bigNumber_red.drawString(g, array5[i], GameScr.xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + num5 + 1, mFont.CENTER);
                            }
                        }
                        else
                        {
                            ...
                            if (flag15)
                            {
                                ...
                                mFont.bigNumber_green.drawString(g, array9[i], GameScr.xSkill + GameScr.xS[i] + 12, GameScr.yS[i] - 10 + 6, mFont.CENTER);
                            }
                        }
                        Skill skill = array[i];
                        bool flag16 = skill != Char.myCharz().myskill;
                        if (flag16)
                        {
                        ...
                        if (!flag17)
                        {
                            bool flag18 = skill == Char.myCharz().myskill;
                            if (flag18)
                            {
                            ...
                            bool flag20 = (i == selectedIndexSkill && !isPaintUI() && GameCanvas.gameTick % 10 > 5) || i == keyTouchSkill;
                            if (flag20)
                            {
                            ...
                            mFont.bigNumber_yellow.drawString(g, (num6 > 0) ? string.Concat(num6 / 1000) : string.Empty, xSkill + xS[i] + 14, yS[i] + 8, mFont.CENTER, mFont.tahoma_7b_yellow);

                        }
                    }
                }
                PaintGameScr.getInstance().paintGamePad(g);
            }
        }
    }
    ...
    public bool isNotPaintTouchControl()
    {
        ...
        return ChatTextField.gI().isShow || InfoDlg.isShow || GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu || GameCanvas.panel.isShow || isPaintPopup();
    }

    public bool isPaintUI()
    {
        return isPaintStore || isPaintWeapon || isPaintNonNam || isPaintNonNu || isPaintAoNam || isPaintAoNu || isPaintGangTayNam || isPaintGangTayNu || isPaintQuanNam || isPaintQuanNu || isPaintGiayNam || isPaintGiayNu || isPaintLien || isPaintNhan || isPaintNgocBoi || isPaintPhu || isPaintStack || isPaintStackLock || isPaintGrocery || isPaintGroceryLock || isPaintUpGrade || isPaintConvert || isPaintSplit || isPaintUpPearl || isPaintBox || isPaintTrade;
    }

    public bool isOpenUI()
    {
        return isPaintItemInfo || isPaintInfoMe || isPaintStore || isPaintNonNam || isPaintNonNu || isPaintAoNam || isPaintAoNu || isPaintGangTayNam || isPaintGangTayNu || isPaintQuanNam || isPaintQuanNu || isPaintGiayNam || isPaintGiayNu || isPaintLien || isPaintNhan || isPaintNgocBoi || isPaintPhu || isPaintWeapon || isPaintStack || isPaintStackLock || isPaintGrocery || isPaintGroceryLock || isPaintUpGrade || isPaintConvert || isPaintUpPearl || isPaintBox || isPaintSplit || isPaintTrade;
    }
    ...
    public void paintTitle(mGraphics g, string title, bool arrow)
    {
        ...
        mFont.bigNumber_While.drawString(g, title, num, popupY + 9, 2);
    }

    public static int getTaskMapId()
    {
        return Char.myCharz().taskMaint == null ? -1 : mapTasks[Char.myCharz().taskMaint.index];
    }
    ...
    public void giaodich(int playerID)
    {
        ...
        //{
        //    Char @char = findCharInMap(playerID);
        //    if (@char != null && Sellitem.isBan)
        //    {
        //        Service.gI().giaodich(1, @char.charID, -1, -1);
        //        new Thread(Sellitem.autoBanitem).Start();
        //    }
        //}
        //else
        //{
    ...
    private static void setTouchBtn()
    {
        if (isAnalog != 0)
        {
            ...
            if (gamePad.isLargeGamePad)
            {
                ...
                wSkill = 35;
                int n = GameCanvas.w / wSkill;
                int pos = n - maxSkill;
                xSkill = pos % 2 == 0 ? wSkill * pos / 2 : (wSkill * pos / 2) + (wSkill / 2);
                xSkill = (xSkill > 17 || xSkill >= GameCanvas.w / 2) ? xSkill : 17;
                xHP = xF - 45;
            }
    ...
    public void chatVip(string chatVip)
    {
        ...
        BossCustom.getBossCustom.addBoss(chatVip);
        if (!startChat)
        {
    ...
    public void paintChatVip(mGraphics g)
    {
        if (vChatVip.size() != 0 && isPaintChatVip)
        {
            ...
            g.fillRect(0, GameCanvas.h - 13, GameCanvas.w, 15, UpdateGameScr.mau1[7], 1);
            string st = (string)vChatVip.elementAt(0);
            mFont.tahoma_7b_yellow.drawStringBd(g, st, xChatVip, GameCanvas.h - 13, 0, mFont.tahoma_7b_dark);
            //  Debug.Log(st);
        }
    }
    ...
    {
        return TileMap.mapPhuBang() && phuban_Info.type_PB == 0;
    }
    ...
    public static void paintHPBar_NEW(mGraphics g, int x, int y, Char c)
    {
        ...
        int num2 = c.cHP * width / c.cHPFull;
        if (num2 <= 0)
        {
            num2 = 1;
        }
        ...
        int num3 = c.cMP * width / c.cMPFull;
        if (num3 <= 0)
        {
            num3 = 1;
        }
    ...
    {
        Effect_End eff = new(type, subtype, typePaint, charUse, target, levelPaint, timeRemove, range);
        addEffect2Vector(eff);
    }
    ...
    {
        return x >= cmx - wOne && x <= cmx + GameCanvas.w + wOne && y >= cmy - hOne && y <= cmy + GameCanvas.h + (hOne * 3 / 2);
    }
