...
public class Menu
{
    ...
    private readonly Command right = new(mResources.CLOSE, 0, GameCanvas.w - 71, GameCanvas.h - mScreen.cmdH + 1);

    private readonly Command center;
    ...
    public void startAt(MyVector menuItems, int pos)
    {
        ...
        if (menuItems.size() != 0)
        {
            ...
            for (int j = 0; j < menuItems.size(); j++)
            {
                ...
                _ = mFont.tahoma_7_yellow.getWidth(command2.caption);
    ...
    {
        return (!isClose && menuTemY[^1] > menuY) || (isClose && menuTemY[^1] < GameCanvas.h);
    }
    ...
    public void paintMenu(mGraphics g)
    {
        ...
        for (int i = 0; i < menuItems.size(); i++)
        {
            ...
            for (int j = 0; j < array.Length; j++)
            {
                if (i == menuSelectedItem)
                {
                    mFont.tahoma_7b_red.drawString(g, array[j], menuX + (i * menuW) + (menuW / 2), num + (j * 14), 2, mFont.tahoma_7b_dark);
                }
                else
                {
                    mFont.tahoma_7b_yellow.drawStringBd(g, array[j], menuX + (i * menuW) + (menuW / 2), num + (j * 14), 2, mFont.tahoma_7b_dark);
                }
            }
        }
        g.translate(-g.getTranslateX(), -g.getTranslateY());
